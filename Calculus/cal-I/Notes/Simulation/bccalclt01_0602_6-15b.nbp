(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 8.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     21083,        443]
NotebookOptionsPosition[      8382,        172]
NotebookOutlinePosition[     21177,        445]
CellTagsIndexPosition[     21134,        442]
WindowTitle->Section 6.2, Figure 6.15b
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`xValue$$ = -0.2499999999999999, 
    Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`xValue$$], -0.75, 
       "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, -0.95, 3.95, 
      0.1}}, Typeset`size$$ = {432., {137., 142.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`xValue$136106$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, "Variables" :> {$CellContext`xValue$$ = -0.75}, 
      "ControllerVariables" :> {
        Hold[$CellContext`xValue$$, $CellContext`xValue$136106$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Show[{
         Plot[{
           $CellContext`funcLTC6F15f[$CellContext`x], 
           $CellContext`funcLTC6F15g[$CellContext`x]}, {$CellContext`x, -2.5, 
           4}, PlotStyle -> {{Thick, Black}, {Thick, $CellContext`bcR}}], 
         Plot[{
           $CellContext`funcLTC6F15f[$CellContext`x], 
           $CellContext`funcLTC6F15g[$CellContext`x]}, {$CellContext`x, -1, 
           3}, PlotStyle -> {{Thick, Black}, {Thick, $CellContext`bcR}}, 
          Filling -> {1 -> {2}}, 
          FillingStyle -> $CellContext`bcAreaPositiveColor], 
         If[$CellContext`xValue$$ <= 0, 
          Plot[{
            $CellContext`funcLTC6F15f[$CellContext`x], 
            $CellContext`funcLTC6F15g[$CellContext`x]}, {$CellContext`x, -1, 
            0}, PlotStyle -> {{Thick, Black}, {Thick, $CellContext`bcR}}, 
           Filling -> {1 -> {2}}, FillingStyle -> 
           Darker[$CellContext`bcAreaPositiveColor, 0.2], Epilog -> {
             Text[
             "\!\(\*FormBox[SubscriptBox[\"R\", \"1\"],\n \
TraditionalForm]\)", {-0.5, 1.5}]}], 
          Plot[{
            $CellContext`funcLTC6F15f[$CellContext`x], 
            $CellContext`funcLTC6F15g[$CellContext`x]}, {$CellContext`x, 0, 
            3}, PlotStyle -> {{Thick, Black}, {Thick, $CellContext`bcR}}, 
           Filling -> {1 -> {2}}, FillingStyle -> 
           Darker[$CellContext`bcAreaPositiveColor, 0.2]]], 
         Graphics[{Gray, 
           Arrowheads[{0, 0.025}], 
           If[$CellContext`xValue$$ <= 0, 
            Arrow[{{-2.5, 6}, {-0.5, 1.8}}], 
            Arrow[{{-2.5, 6}, {0.5, 2.7}}]], $CellContext`bcAreaNegativeColor, 
           EdgeForm[$CellContext`bcAreaNegativeBorderColor], 
           Rectangle[{$CellContext`xValue$$ - 0.075, 
             $CellContext`funcLTC6F15g[$CellContext`xValue$$]}, \
{$CellContext`xValue$$ + 0.075, 
             $CellContext`funcLTC6F15f[$CellContext`xValue$$]}], Black, 
           AbsolutePointSize[6], 
           Point[{{-1, 2}, {3, 6}}], 
           Text[
           "\!\(\*FormBox[SubscriptBox[\"R\", \"1\"],\n TraditionalForm]\)", \
{-0.3, 1.5}], 
           Text[
           "\!\(\*FormBox[SubscriptBox[\"R\", \"2\"],\n TraditionalForm]\)", {
            0.7, 2.5}], 
           Text[
           "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\n   RowBox[{\"-\", \
\"1\"}], \",\", \"2\"}], \")\"}],\n TraditionalForm]\)", {-1, 2}, {0.5, 3}], 
           Text[
           "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"3\", \",\", \
\"6\"}], \")\"}],\n TraditionalForm]\)", {3, 6}, {-0.7, 3}], Black, 
           If[$CellContext`xValue$$ <= 0, 
            Text[
             Framed[
              Pane[
               
               Row[{"Area of \!\(\*SubscriptBox[\(R\), \(1\)]\)", "\n", 
                 "\!\(\*FormBox[\n RowBox[{\"\[InvisiblePostfixScriptBase]\", \
\"=\", \n  RowBox[{SubsuperscriptBox[\"\[Integral]\", \n    RowBox[{\" \", \n \
    RowBox[{\"-\", \"1\"}]}], \n    RowBox[{\" \", \"0\"}]], \n   RowBox[{\n  \
  RowBox[{\"[\", \n     RowBox[{\n      RowBox[{\"(\", \n       \
RowBox[{\"x\", \"+\", \"3\"}], \")\"}], \"-\", \n      RowBox[{\"(\", \n      \
 RowBox[{\n        RowBox[{\"-\", \"2\"}], \"x\"}], \")\"}]}], \"]\"}], \" \
\", \"d\", \"\[VeryThinSpace]\", \"x\"}]}]}],\n TraditionalForm]\)"}], \
$CellContext`bcPBS], $CellContext`bcFO, Background -> White], {-4, 8}, {-1, 
             1}], 
            Text[
             Framed[
              Pane[
               
               Row[{"Area of \!\(\*SubscriptBox[\(R\), \(2\)]\)", "\n", 
                 "\!\(\*FormBox[\n RowBox[{\"\[InvisiblePostfixScriptBase]\", \
\"=\", \n  RowBox[{SubsuperscriptBox[\"\[Integral]\", \n    RowBox[{\" \", \
\"0\"}], \n    RowBox[{\" \", \"3\"}]], \n   RowBox[{\n    RowBox[{\"[\", \n  \
   RowBox[{\n      RowBox[{\"(\", \n       RowBox[{\"x\", \"+\", \"3\"}], \")\
\"}], \"-\", \n      RowBox[{\"(\", \n       RowBox[{\"2\", \"x\"}], \
\")\"}]}], \"]\"}], \" \", \"d\", \"\[VeryThinSpace]\", \"x\"}]}]}],\n \
TraditionalForm]\)"}], $CellContext`bcPBS], $CellContext`bcFO, Background -> 
              White], {-4, 8}, {-1, 1}]], 
           Text[
           "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"f\", \"(\", \"x\", \
\")\"}], \"=\", \n  RowBox[{\"x\", \"+\", \"3\"}]}],\n TraditionalForm]\)", {
            2.5, 
             $CellContext`funcLTC6F15f[2.5]}, {1.5, -1}], 
           Text[
           "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"g\", \"(\", \"x\", \
\")\"}], \"=\", FormBox[\n   RowBox[{\"\[LeftBracketingBar]\", \n    RowBox[{\
\"2\", \"x\"}], \"\[RightBracketingBar]\"}],\n   TraditionalForm]}],\n \
TraditionalForm]\)", {1.5, 
             $CellContext`funcLTC6F15g[1.5]}, {-1.5, 1}]}]}, ImageSize -> 
        6 72, BaseStyle -> {"Text", 13}, Ticks -> {
          Range[-3, 3], 
          Range[0, 7]}, AxesStyle -> Arrowheads[{0, 0.03}], 
        AxesLabel -> {
         "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", 
          "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)"}, 
        PlotRange -> {{-4, 4}, {-0.5, 8}}], 
      "Specifications" :> {{{$CellContext`xValue$$, -0.75, 
          "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, -0.95, 3.95, 
         0.1, ControlType -> Slider, ImageSize -> Small}}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{599., {168., 173.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{$CellContext`funcLTC6F15f[
           Pattern[$CellContext`x, 
            Blank[]]] := $CellContext`x + 3, $CellContext`funcLTC6F15g[
           Pattern[$CellContext`x, 
            Blank[]]] := Abs[2 $CellContext`x], $CellContext`bcR = 
         RGBColor[0.8, 0, 0.4], $CellContext`bcAreaPositiveColor = 
         RGBColor[0.796, 0.91, 0.855], $CellContext`bcAreaNegativeColor = 
         RGBColor[
          0.976, 0.871, 0.812], $CellContext`bcAreaNegativeBorderColor = 
         RGBColor[0.796, 0, 0.42], $CellContext`bcPBS = 
         BaseStyle -> {
           LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
            LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
            Left, FontSize -> 13}, $CellContext`bcFO = {
          RoundingRadius -> 5, FrameStyle -> GrayLevel[0.85]}, 
         Attributes[PlotRange] = {ReadProtected}}; {Null, Null}}; 
     Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1354, 874},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 6.2, Figure 6.15b",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"6.2 Regions Between Curves\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox[
      "Section 6.2 Regions Between Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox[
      "Section 6.2 Regions Between Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
   StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[591, 21, 7787, 149, 358, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature mJ4CRjjWX5erkI5sR0NAO4VW *)
