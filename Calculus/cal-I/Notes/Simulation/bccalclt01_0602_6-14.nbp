(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 8.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     25154,        514]
NotebookOptionsPosition[     12453,        243]
NotebookOutlinePosition[     25247,        516]
CellTagsIndexPosition[     25204,        513]
WindowTitle->Section 6.2, Figure 6.14
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`aValue$$ = 0, $CellContext`bValue$$ = 
    3, $CellContext`kValue$$ = 3, $CellContext`nValue$$ = 10, Typeset`show$$ =
     True, Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`aValue$$], 0, 
       "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, -0.45, 0.9}, {{
       Hold[$CellContext`bValue$$], 3, 
       "\!\(\*\nStyleBox[\"b\",\nFontSlant->\"Italic\"]\)"}, 
      Dynamic[$CellContext`aValue$$ + 0.01], 1.3}, {{
       Hold[$CellContext`nValue$$], 10, 
       "\!\(\*\nStyleBox[\"n\",\nFontSlant->\"Italic\"]\)"}, 2, 20, 1}, {{
       Hold[$CellContext`kValue$$], 3, 
       "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, 1, 
      Dynamic[$CellContext`nValue$$], 1}}, Typeset`size$$ = {
    360., {180., 186.}}, Typeset`update$$ = 0, Typeset`initDone$$, 
    Typeset`skipInitDone$$ = False, $CellContext`aValue$136024$$ = 
    0, $CellContext`bValue$136025$$ = 0, $CellContext`nValue$136026$$ = 
    0, $CellContext`kValue$136027$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`aValue$$ = 0, $CellContext`bValue$$ = 
        3, $CellContext`kValue$$ = 3, $CellContext`nValue$$ = 10}, 
      "ControllerVariables" :> {
        Hold[$CellContext`aValue$$, $CellContext`aValue$136024$$, 0], 
        Hold[$CellContext`bValue$$, $CellContext`bValue$136025$$, 0], 
        Hold[$CellContext`nValue$$, $CellContext`nValue$136026$$, 0], 
        Hold[$CellContext`kValue$$, $CellContext`kValue$136027$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, 
      "Body" :> (
       If[$CellContext`aValue$$ >= $CellContext`bValue$$, \
$CellContext`bValue$$ = $CellContext`aValue$$ + 0.01]; 
       If[$CellContext`kValue$$ > $CellContext`nValue$$, \
$CellContext`kValue$$ = $CellContext`nValue$$]; 
       Module[{$CellContext`\[CapitalDelta]x$}, \
$CellContext`\[CapitalDelta]x$ = ($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$; Show[{
            Plot[{
              $CellContext`funcLTC6F14f[$CellContext`x], 
              $CellContext`funcLTC6F14g[$CellContext`x]}, {$CellContext`x, \
$CellContext`aValue$$, $CellContext`bValue$$}, 
             PlotStyle -> {{Thick, Black}, {Thick, $CellContext`bcR}}, 
             Filling -> {1 -> {2}}, 
             FillingStyle -> $CellContext`bcAreaPositiveColor], 
            Graphics[{
              
              EdgeForm[$CellContext`bcAreaNegativeBorderColor], \
$CellContext`bcAreaNegativeColor, 
              
              Rectangle[{$CellContext`aValue$$ + ($CellContext`kValue$$ - 
                  1) $CellContext`\[CapitalDelta]x$, 
                $CellContext`funcLTC6F14g[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                   2) $CellContext`\[CapitalDelta]x$]}, \
{$CellContext`aValue$$ + $CellContext`kValue$$ $CellContext`\[CapitalDelta]x$, 
                $CellContext`funcLTC6F14f[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/2) $CellContext`\[CapitalDelta]x$]}]}], 
            Plot[{
              $CellContext`funcLTC6F14f[$CellContext`x], 
              $CellContext`funcLTC6F14g[$CellContext`x]}, {$CellContext`x, \
-0.5, 4}, PlotStyle -> {{Thick, Black}, {Thick, $CellContext`bcR}}]}, 
           PlotRange -> {{-0.5, 4}, {-3, 6}}, AspectRatio -> 1, Axes -> True, 
           AxesOrigin -> {0, 0}, ImageSize -> 5 72, Ticks -> {
             Range[0, 4], 
             Range[-2, 4, 2]}, BaseStyle -> {"Text", 13}, AxesStyle -> 
           Arrowheads[{0, 0.03}], 
           AxesLabel -> {
            "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", 
             "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)"}, 
           Epilog -> {
             AbsolutePointSize[5], Black, 
             Point[{$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                 2) $CellContext`\[CapitalDelta]x$, 
               $CellContext`funcLTC6F14f[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/2) $CellContext`\[CapitalDelta]x$]}], 
             
             Point[{$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                 2) $CellContext`\[CapitalDelta]x$, 
               $CellContext`funcLTC6F14g[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/2) $CellContext`\[CapitalDelta]x$]}], 
             Text[
              Pane[
              "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"f\", \"(\", \"x\", \
\")\"}], \"-\", \n  RowBox[{\"g\", \"(\", \"x\", \")\"}]}],\n \
TraditionalForm]\)", $CellContext`bcPBS], {$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1) $CellContext`\[CapitalDelta]x$ - 
               0.05, ($CellContext`funcLTC6F14f[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                    2) $CellContext`\[CapitalDelta]x$] + \
$CellContext`funcLTC6F14g[$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                    2) $CellContext`\[CapitalDelta]x$])/2}, {1.1, 0}], Black, 
             
             Arrowheads[{-0.03, 0.03}], 
             
             Arrow[{{$CellContext`aValue$$ + ($CellContext`kValue$$ - 
                  1) $CellContext`\[CapitalDelta]x$ - 0.03, 
                $CellContext`funcLTC6F14f[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                   2) $CellContext`\[CapitalDelta]x$]}, \
{$CellContext`aValue$$ + ($CellContext`kValue$$ - 
                  1) $CellContext`\[CapitalDelta]x$ - 0.03, 
                $CellContext`funcLTC6F14g[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/2) $CellContext`\[CapitalDelta]x$]}}], 
             Arrowheads[{-0.02, 0.02}], 
             
             Arrow[{{$CellContext`aValue$$ + ($CellContext`kValue$$ - 
                  1) $CellContext`\[CapitalDelta]x$, \
$CellContext`funcLTC6F14g[$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                    2) $CellContext`\[CapitalDelta]x$] - 
                0.05}, {$CellContext`aValue$$ + $CellContext`kValue$$ \
$CellContext`\[CapitalDelta]x$, \
$CellContext`funcLTC6F14g[$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                    2) $CellContext`\[CapitalDelta]x$] - 0.05}}], Black, 
             Point[{3, 2}], 
             Text[
             "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"3\", \",\", \
\"2\"}], \")\"}],\n TraditionalForm]\)", {3, 2}, {-3, -1}], 
             Text[
              Pane[
              "\!\(\*FormBox[\n RowBox[{\"\[CapitalDelta]\", \"\
\[VeryThinSpace]\", \"x\"}],\n TraditionalForm]\)", $CellContext`bcPBS], \
{$CellContext`aValue$$ + ($CellContext`kValue$$ - 
                 1) $CellContext`\[CapitalDelta]x$, 
               $CellContext`funcLTC6F14g[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1) $CellContext`\[CapitalDelta]x$]}, {1.5, 0}], 
             Text[
              Pane[
              "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"x\", \",\", \n   \
RowBox[{\"f\", \"(\", \"x\", \")\"}]}], \")\"}],\n TraditionalForm]\)", \
$CellContext`bcPBS], {$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                 2) $CellContext`\[CapitalDelta]x$, 
               $CellContext`funcLTC6F14f[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/2) $CellContext`\[CapitalDelta]x$]}, {0, -3}], 
             Text[
              Pane[
              "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"x\", \",\", \n   \
RowBox[{\"g\", \"(\", \"x\", \")\"}]}], \")\"}],\n TraditionalForm]\)", \
$CellContext`bcPBS], {$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                 2) $CellContext`\[CapitalDelta]x$, 
               $CellContext`funcLTC6F14g[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/2) $CellContext`\[CapitalDelta]x$]}, {0, 3}], 
             Text[
              Framed[
               Pane[
               "\!\(\*FormBox[GridBox[{\n   {\n    RowBox[{\n     \
RowBox[{\"Rectangle\", \" \", \"height\"}], \"=\", \n     RowBox[{\n      \
RowBox[{\"f\", \"(\", \"x\", \")\"}], \"-\", \n      RowBox[{\"g\", \"(\", \
\"x\", \")\"}]}]}]},\n   {\n    RowBox[{\n     RowBox[{\"Rectangle\", \" \", \
\"width\"}], \"=\", \n     RowBox[{\"\[CapitalDelta]\", \"\[VeryThinSpace]\", \
\"x\"}]}]}\n  },\nGridBoxAlignment->{\"Columns\" -> {{\"=\"}}}],\n \
TraditionalForm]\)", $CellContext`bcPBS], $CellContext`bcFO, Background -> 
               White], {0.5, 6}, {-1.2, 1}], 
             Text[
              Framed[
               Pane[
               "Slice the region using vertical rectangles from \
\!\(\*FormBox[\n RowBox[{\"x\", \"=\", \"0\"}],\n TraditionalForm]\) to \
\!\(\*FormBox[\n RowBox[{\"x\", \"=\", \"3\"}],\n TraditionalForm]\).", {
                180, Automatic}, $CellContext`bcPBS], $CellContext`bcFO, 
               Background -> White], {0.5, -3}, {-1, -1}], 
             Text[
             "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"f\", \"(\", \"x\", \
\")\"}], \"=\", FractionBox[\"4\", SqrtBox[\n    RowBox[{\"x\", \"+\", \
\"1\"}]]]}],\n TraditionalForm]\)", {1.5, 
               $CellContext`funcLTC6F14f[1.5]}, {-1, -1}], 
             Text[
             "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"g\", \"(\", \"x\", \
\")\"}], \"=\", \n  RowBox[{\"x\", \"-\", \"1\"}]}],\n TraditionalForm]\)", {
              2, 
               $CellContext`funcLTC6F14g[2]}, {-1, 1}]}]]), 
      "Specifications" :> {{{$CellContext`aValue$$, 0, 
          "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, -0.45, 0.9, 
         ControlType -> None, ImageSize -> 
         Small}, {{$CellContext`bValue$$, 3, 
          "\!\(\*\nStyleBox[\"b\",\nFontSlant->\"Italic\"]\)"}, 
         Dynamic[$CellContext`aValue$$ + 0.01], 1.3, ControlType -> None, 
         ImageSize -> Small}, 
        Delimiter, {{$CellContext`nValue$$, 10, 
          "\!\(\*\nStyleBox[\"n\",\nFontSlant->\"Italic\"]\)"}, 2, 20, 1, 
         ControlType -> None, ImageSize -> 
         Small}, {{$CellContext`kValue$$, 3, 
          "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, 1, 
         Dynamic[$CellContext`nValue$$], 1, ControlType -> Slider, ImageSize -> 
         Small}}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{527., {211., 216.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{$CellContext`\[CapitalDelta]x = 
         1/100, $CellContext`funcLTC6F14f[
           Pattern[$CellContext`x, 
            Blank[]]] := 4/Sqrt[$CellContext`x + 1], $CellContext`funcLTC6F14g[
           Pattern[$CellContext`x, 
            Blank[]]] := $CellContext`x - 1, $CellContext`bcR = 
         RGBColor[0.8, 0, 0.4], $CellContext`bcAreaPositiveColor = 
         RGBColor[0.796, 0.91, 0.855], $CellContext`bcAreaNegativeBorderColor = 
         RGBColor[0.796, 0, 0.42], $CellContext`bcAreaNegativeColor = 
         RGBColor[0.976, 0.871, 0.812], 
         Attributes[PlotRange] = {ReadProtected}, $CellContext`bcPBS = 
         BaseStyle -> {
           LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
            LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
            Left, FontSize -> 13}, $CellContext`bcFO = {
          RoundingRadius -> 5, FrameStyle -> GrayLevel[0.85]}}; {Null, Null}}; 
     Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1354, 874},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 6.2, Figure 6.14",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"6.2 Regions Between Curves\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox[
      "Section 6.2 Regions Between Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox[
      "Section 6.2 Regions Between Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
   StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[590, 21, 11859, 220, 444, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature oLNxQWVHvAyrLK#M8pghfPuL *)
