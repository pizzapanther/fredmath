(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 8.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     26239,        550]
NotebookOptionsPosition[     13567,        281]
NotebookOutlinePosition[     26332,        552]
CellTagsIndexPosition[     26289,        549]
WindowTitle->Section 6.3, Figure 6.31
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`showSurface$$ = 
    False, $CellContext`xValue$$ = 0.5, $CellContext`\[Theta]Max$$ = 2 Pi, 
    Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`xValue$$], 0.5, 
       "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, 0, 1.4, 0.1}, {{
       Hold[$CellContext`\[Theta]Max$$], 2 Pi, "revolve"}, 0.01, 2 Pi}, {{
       Hold[$CellContext`showSurface$$], False, "show solid"}, {
      True, False}}, {
      Hold[
       Row[{
         Manipulate`Place[1], 
         Spacer[10], 
         Manipulate`Place[2], 
         Spacer[10], 
         Manipulate`Place[3]}]], Manipulate`Dump`ThisIsNotAControl}}, 
    Typeset`size$$ = {360., {192.5, 199.5}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`xValue$137050$$ = 
    0, $CellContext`\[Theta]Max$137051$$ = 
    0, $CellContext`showSurface$137052$$ = False}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`showSurface$$ = 
        False, $CellContext`xValue$$ = 0.5, $CellContext`\[Theta]Max$$ = 
        2 Pi}, "ControllerVariables" :> {
        Hold[$CellContext`xValue$$, $CellContext`xValue$137050$$, 0], 
        Hold[$CellContext`\[Theta]Max$$, $CellContext`\[Theta]Max$137051$$, 
         0], 
        Hold[$CellContext`showSurface$$, $CellContext`showSurface$137052$$, 
         False]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Column[{
         Framed[
          Pane[
          "Cross-sectional area \!\(\*FormBox[\n RowBox[{\"\
\[InvisiblePostfixScriptBase]\", \"=\", \n  RowBox[{\n   RowBox[{\n    \
RowBox[{\"\[Pi]\", \" \", SuperscriptBox[\n      RowBox[{\"f\", \n       \
RowBox[{\"(\", \"x\", \")\"}]}], \"2\"]}], \"-\", \n    RowBox[{\"\[Pi]\", \" \
\", SuperscriptBox[\n      RowBox[{\"g\", \n       RowBox[{\"(\", \"x\", \
\")\"}]}], \"2\"]}]}], \"=\", \n   RowBox[{\"\[Pi]\", \" \", \"[\", \" \", \n \
   RowBox[{SuperscriptBox[\n      RowBox[{\"f\", \n       RowBox[{\"(\", \
\"x\", \")\"}]}], \"2\"], \"-\", SuperscriptBox[\n      RowBox[{\"g\", \n     \
  RowBox[{\"(\", \"x\", \")\"}]}], \"2\"]}], \"]\"}]}]}],\n \
TraditionalForm]\)", {
           Automatic, Automatic}, $CellContext`bcPBS], $CellContext`bcFO], 
         Show[{
           ParametricPlot3D[{{$CellContext`x, 0, 
              $CellContext`funcLTC6F31a[$CellContext`x]}, {$CellContext`x, 
              0, 
              $CellContext`funcLTC6F31b[$CellContext`x]}}, {$CellContext`x, 0,
              1.5}, PlotStyle -> {{Thick, Black}}], 
           
           ParametricPlot3D[{{$CellContext`x, 
              0, (1 - $CellContext`u) \
$CellContext`funcLTC6F31a[$CellContext`x] + $CellContext`u \
$CellContext`funcLTC6F31b[$CellContext`x]}, {$CellContext`x, 
              Sin[$CellContext`\[Theta]Max$$] ((
                 1 - $CellContext`u) \
$CellContext`funcLTC6F31a[$CellContext`x] + $CellContext`u \
$CellContext`funcLTC6F31b[$CellContext`x]), 
              Cos[$CellContext`\[Theta]Max$$] ((
                 1 - $CellContext`u) \
$CellContext`funcLTC6F31a[$CellContext`x] + $CellContext`u \
$CellContext`funcLTC6F31b[$CellContext`x])}}, {$CellContext`x, 0.3, 
             1.4}, {$CellContext`u, 0, 1}, PlotPoints -> 20, MaxRecursion -> 
            0, PlotStyle -> {{
               Opacity[1], $CellContext`bcAreaPositiveColor}, {
               Opacity[1], $CellContext`bcAreaPositiveColor}}, Mesh -> None], 
           
           
           ParametricPlot3D[{$CellContext`xValue$$, 
             Sin[(($CellContext`x - 0.3)/1.1) $CellContext`\[Theta]Max$$] ((
                1 - $CellContext`u) \
$CellContext`funcLTC6F31a[$CellContext`xValue$$] + $CellContext`u \
$CellContext`funcLTC6F31b[$CellContext`xValue$$]), 
             Cos[(($CellContext`x - 0.3)/1.1) $CellContext`\[Theta]Max$$] ((
                1 - $CellContext`u) \
$CellContext`funcLTC6F31a[$CellContext`xValue$$] + $CellContext`u \
$CellContext`funcLTC6F31b[$CellContext`xValue$$])}, {$CellContext`x, 0.3, 
             1.4}, {$CellContext`u, 0, 1}, PlotPoints -> 25, MaxRecursion -> 
            0, PlotStyle -> {
              Opacity[1], $CellContext`bcB}, Mesh -> None], 
           If[$CellContext`showSurface$$, {
             
             ParametricPlot3D[{
              0.3, Sin[(($CellContext`x - 0.3)/
                  1.1) $CellContext`\[Theta]Max$$] ((
                  1 - $CellContext`u) $CellContext`funcLTC6F31a[
                  0.3] + $CellContext`u $CellContext`funcLTC6F31b[0.3]), 
               Cos[(($CellContext`x - 0.3)/1.1) $CellContext`\[Theta]Max$$] ((
                  1 - $CellContext`u) $CellContext`funcLTC6F31a[
                  0.3] + $CellContext`u $CellContext`funcLTC6F31b[
                  0.3])}, {$CellContext`x, 0.3, 1.4}, {$CellContext`u, 0, 1}, 
              PlotPoints -> 20, MaxRecursion -> 0, PlotStyle -> 
              ControlActive[
                FaceForm[{
                  Opacity[1], 
                  Darker[$CellContext`bcAreaPositiveColor]}, {
                  Opacity[1], $CellContext`bcSurfaceTop}], 
                FaceForm[{
                  Opacity[0.3], 
                  Darker[$CellContext`bcAreaPositiveColor]}, {
                  Opacity[0.3], $CellContext`bcSurfaceTop}]], Mesh -> None], 
             
             ParametricPlot3D[{
              1.4, Sin[(($CellContext`x - 0.3)/
                  1.1) $CellContext`\[Theta]Max$$] ((
                  1 - $CellContext`u) $CellContext`funcLTC6F31a[
                  1.4] + $CellContext`u $CellContext`funcLTC6F31b[1.4]), 
               Cos[(($CellContext`x - 0.3)/1.1) $CellContext`\[Theta]Max$$] ((
                  1 - $CellContext`u) $CellContext`funcLTC6F31a[
                  1.4] + $CellContext`u $CellContext`funcLTC6F31b[
                  1.4])}, {$CellContext`x, 0.3, 1.4}, {$CellContext`u, 0, 1}, 
              PlotPoints -> 20, MaxRecursion -> 0, PlotStyle -> 
              ControlActive[
                FaceForm[{
                  Opacity[1], $CellContext`bcSurfaceTop}, {
                  Opacity[1], 
                  Darker[$CellContext`bcAreaPositiveColor]}], 
                FaceForm[{
                  Opacity[0.3], $CellContext`bcSurfaceTop}, {
                  Opacity[0.3], 
                  Darker[$CellContext`bcAreaPositiveColor]}]], Mesh -> None], 
             
             RevolutionPlot3D[{{$CellContext`x, 0, 
                $CellContext`funcC6F30a[$CellContext`x]}, {$CellContext`x, 
                0, 
                $CellContext`funcC6F30b[$CellContext`x]}}, {$CellContext`x, 
               0.3, 1.4}, {$CellContext`\[Theta], -$CellContext`\[Theta]Max$$,
                0}, RevolutionAxis -> {1, 0, 0}, Mesh -> None, PlotPoints -> 
              20, MaxRecursion -> 1, PlotStyle -> {
                ControlActive[
                 FaceForm[{
                   Opacity[1], $CellContext`bcSurfaceTop}, {
                   Opacity[1], 
                   Darker[$CellContext`bcAreaPositiveColor]}], 
                 FaceForm[{
                   Opacity[0.3], $CellContext`bcSurfaceTop}, {
                   Opacity[0.3], 
                   Darker[$CellContext`bcAreaPositiveColor]}]], 
                ControlActive[
                 FaceForm[{
                   Opacity[1], 
                   Darker[$CellContext`bcAreaPositiveColor]}, {
                   Opacity[1], $CellContext`bcSurfaceTop}], 
                 FaceForm[{
                   Opacity[0.3], 
                   Darker[$CellContext`bcAreaPositiveColor]}, {
                   Opacity[0.3], $CellContext`bcSurfaceTop}]]}]}, 
            Graphics3D[{}]], 
           Graphics3D[{
             Text[
             "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)", {
              0.3, 0, 0}, {0, 1.5}], 
             Text[
             "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", \
{$CellContext`xValue$$, 0, 0}, {0, 1.5}], 
             Text[
             "\!\(\*\nStyleBox[\"b\",\nFontSlant->\"Italic\"]\)", {
              1.4, 0, 0}, {0, 1.5}], 
             Text[
             "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", {2, 0, 0}, {
              0, -1}], 
             Text[
             "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)", {
              0, 0, 1.1}, {0, -1}], 
             Line[{{-0.5, 0, 0}, {2.5, 0, 0}}], 
             Line[{{0, 0, -1}, {0, 0, 1}}], 
             Text[
             "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"x\", \",\", \n   \
RowBox[{\"g\", \"(\", \"x\", \")\"}]}], \")\"}],\n TraditionalForm]\)", {
              1.4, 0, 0.8}, {-1, -1}], 
             Text[
             "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"x\", \",\", \n   \
RowBox[{\"f\", \"(\", \"x\", \")\"}]}], \")\"}],\n TraditionalForm]\)", {
              1.2, 0, 1.2}, {-1, -1}], 
             GrayLevel[0.3], 
             Line[{{1.4, 0, 0.8}, {$CellContext`xValue$$, 0, 
                $CellContext`funcLTC6F31a[$CellContext`xValue$$]}}], 
             Line[{{1.2, 0, 1.2}, {$CellContext`xValue$$, 0, 
                $CellContext`funcLTC6F31b[$CellContext`xValue$$]}}], 
             AbsolutePointSize[8], Black, 
             Point[{$CellContext`xValue$$, 0, 
               $CellContext`funcLTC6F31a[$CellContext`xValue$$]}], 
             Point[{$CellContext`xValue$$, 0, 
               $CellContext`funcLTC6F31b[$CellContext`xValue$$]}], \
$CellContext`bcR, Thick, 
             Line[{{$CellContext`xValue$$, 0, 
                $CellContext`funcLTC6F31a[$CellContext`xValue$$]}, \
{$CellContext`xValue$$, 0, 
                $CellContext`funcLTC6F31b[$CellContext`xValue$$]}}], 
             
             Line[{{$CellContext`xValue$$, 
                Sin[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F31a[$CellContext`xValue$$], 
                Cos[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F31a[$CellContext`xValue$$]}, {$CellContext`xValue$$, 
                Sin[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F31b[$CellContext`xValue$$], 
                Cos[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F31b[$CellContext`xValue$$]}}]}]}, 
          PlotRange -> {{-0.5, 2}, {-1.1, 1.1}, {-1.1, 1.1}}, 
          AxesOrigin -> {0, 0, 0}, Axes -> False, Boxed -> False, Lighting -> 
          "Neutral", ImageSize -> {5 72, 5 72}, ViewPoint -> {10, -10, 5}, 
          BoxRatios -> {1, 1, 1}, BaseStyle -> {"Text", 13}]}, BaseStyle -> 
        "TR", Alignment -> Center], 
      "Specifications" :> {{{$CellContext`xValue$$, 0.5, 
          "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, 0, 1.4, 0.1, 
         ControlType -> Slider, ImageSize -> Small, ControlPlacement -> 
         1}, {{$CellContext`\[Theta]Max$$, 2 Pi, "revolve"}, 0.01, 2 Pi, 
         ControlType -> Slider, ImageSize -> Small, ControlPlacement -> 
         2}, {{$CellContext`showSurface$$, False, "show solid"}, {
         True, False}, ControlType -> Checkbox, ControlPlacement -> 3}, 
        Row[{
          Manipulate`Place[1], 
          Spacer[10], 
          Manipulate`Place[2], 
          Spacer[10], 
          Manipulate`Place[3]}]}, 
      "Options" :> {
       ControlPlacement -> Top, LabelStyle -> 11, AppearanceElements -> 
        "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{409., {240., 245.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{$CellContext`bcPBS = 
         BaseStyle -> {
           LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
            LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
            Left, FontSize -> 13}, $CellContext`bcFO = {
          RoundingRadius -> 5, FrameStyle -> 
           GrayLevel[0.85]}, $CellContext`funcLTC6F31a[
           Pattern[$CellContext`x, 
            Blank[]]] := 
         0.5 ($CellContext`x - 0.9)^2 + 0.15, $CellContext`funcLTC6F31b[
           Pattern[$CellContext`x, 
            Blank[]]] := 
         0.2 Sin[((2 Pi)/2) $CellContext`x] + 
          0.7, $CellContext`bcAreaPositiveColor = 
         RGBColor[0.796, 0.91, 0.855], $CellContext`bcB = 
         RGBColor[0, 0.4, 0.8], $CellContext`bcSurfaceTop = 
         RGBColor[0.85, 0.6, 0.2], $CellContext`bcR = RGBColor[0.8, 0, 0.4], 
         Attributes[PlotRange] = {ReadProtected}}; {Null, Null}}; 
     Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1354, 874},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 6.3, Figure 6.31",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"6.3 Volume by Slicing\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 6.3 Volume by Slicing\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 6.3 Volume by Slicing\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
   StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[590, 21, 12973, 258, 502, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature NLz7gOfxmAg7rLkwKr5hIX0O *)
