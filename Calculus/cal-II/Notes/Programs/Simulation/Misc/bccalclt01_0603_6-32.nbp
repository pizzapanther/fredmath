(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 8.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     25406,        530]
NotebookOptionsPosition[     12734,        261]
NotebookOutlinePosition[     25499,        532]
CellTagsIndexPosition[     25456,        529]
WindowTitle->Section 6.3, Figure 6.32
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`showSurface$$ = 
    False, $CellContext`xValue$$ = 0.5, $CellContext`\[Theta]Max$$ = 0.01, 
    Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`xValue$$], 0.5, 
       "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, 0, 1, 0.1}, {{
       Hold[$CellContext`\[Theta]Max$$], 0.01, "revolve"}, 0.01, 2 Pi}, {{
       Hold[$CellContext`\[Theta]Max$$], 0.01, ""}, 0.01, 2 Pi, 0.1}, {{
       Hold[$CellContext`showSurface$$], False, "show solid"}, {
      True, False}}, {
      Hold[
       Row[{
         Manipulate`Place[1], 
         Spacer[10], 
         Manipulate`Place[2], 
         Spacer[10], 
         Manipulate`Place[3], 
         Spacer[10], 
         Manipulate`Place[4]}]], Manipulate`Dump`ThisIsNotAControl}}, 
    Typeset`size$$ = {522., {156., 161.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`xValue$137177$$ = 
    0, $CellContext`\[Theta]Max$137178$$ = 
    0, $CellContext`showSurface$137179$$ = False}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`showSurface$$ = 
        False, $CellContext`xValue$$ = 0.5, $CellContext`\[Theta]Max$$ = 
        0.01}, "ControllerVariables" :> {
        Hold[$CellContext`xValue$$, $CellContext`xValue$137177$$, 0], 
        Hold[$CellContext`\[Theta]Max$$, $CellContext`\[Theta]Max$137178$$, 
         0], 
        Hold[$CellContext`showSurface$$, $CellContext`showSurface$137179$$, 
         False]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Grid[{{
          Column[{
            Framed[
             Pane[
             "Revolving the region about the \!\(\*FormBox[\"x\",\n \
TraditionalForm]\)-axis produces a bowl-shaped solid. \n\
\!\(\*FormBox[GridBox[{\n   {GridBox[{\n      {\n       RowBox[{\n        \
RowBox[{\"Area\", \" \", \"of\", \" \", \"washer\", \" \", \"face\"}], \"=\", \
\n        RowBox[{\"\[Pi]\", \" \", \"[\", \n         RowBox[{SuperscriptBox[\
\n           RowBox[{\"f\", \"(\", \"x\", \")\"}], \"2\"], \"-\", \
SuperscriptBox[\n           RowBox[{\"g\", \"(\", \"x\", \")\"}], \"2\"]}], \
\"]\"}]}]},\n      {\n       RowBox[{\n        RowBox[{\"\
\[InvisiblePostfixScriptBase]\", \"=\", \n         RowBox[{\"\[Pi]\", \" \", \
SuperscriptBox[\n           RowBox[{\"(\", \n            RowBox[{\"x\", \
\"-\", \"x\"}]}], \"4\"]}]}], \")\"}]}\n     },\nGridBoxAlignment->{\"Columns\
\" -> {{\"=\"}}}]}\n  },\nGridBoxItemSize->{\"Columns\" -> \
{{Scaled[0.96]}}}],\n TraditionalForm]\)", {
              216, Automatic}, $CellContext`bcPBS], $CellContext`bcFO], 
            Show[{
              Plot[{
                $CellContext`funcLTC6F32a[$CellContext`x], 
                $CellContext`funcLTC6F32b[$CellContext`x]}, {$CellContext`x, 
                0, 1.5}, PlotStyle -> {{Thick, Black}}], 
              Plot[{
                $CellContext`funcLTC6F32a[$CellContext`x], 
                $CellContext`funcLTC6F32b[$CellContext`x]}, {$CellContext`x, 
                0, 1}, Filling -> {1 -> {2}}, PlotStyle -> {{Thick, Black}}, 
               FillingStyle -> $CellContext`bcAreaPositiveColor]}, ImageSize -> 
             3 72, AxesOrigin -> {0, 0}, PlotRange -> {{-0.5, 2}, {-0.5, 2}}, 
             AspectRatio -> 1, AxesStyle -> Arrowheads[{0, 0.04}], 
             AxesLabel -> {
              "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", 
               "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)"}, 
             Ticks -> {{
               0, 1, {$CellContext`xValue$$, 
                 "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}}, {0, 
               1}}, BaseStyle -> {"Text", 13}, Epilog -> {
               Text[
               "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"f\", \"(\", \"x\", \")\
\"}], \"=\", SqrtBox[\"x\"]}],\n TraditionalForm]\)", {1, 1}, {-1.5, 1}], 
               Text[
               "\!\(\*FormBox[\"R\",\n TraditionalForm]\)", {0.1, 
                0.1}, {-2, -1}], 
               Text[
               "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"g\", \"(\", \"x\", \")\
\"}], \"=\", SuperscriptBox[\"x\", \"2\"]}],\n TraditionalForm]\)", {1.2, 
                1.5}, {1.5, -1}], $CellContext`bcR, Thick, 
               Line[{{$CellContext`xValue$$, 
                  $CellContext`funcLTC6F32b[$CellContext`xValue$$]}, \
{$CellContext`xValue$$, 
                  $CellContext`funcLTC6F32a[$CellContext`xValue$$]}}], 
               AbsoluteThickness[1.5], 
               Line[{{0, 0}, {1, 0}}]}]}, BaseStyle -> {"TR"}], 
          Show[{
            ParametricPlot3D[{{$CellContext`x, 0, 
               $CellContext`funcLTC6F32a[$CellContext`x]}, {$CellContext`x, 0, 
               $CellContext`funcLTC6F32b[$CellContext`x]}}, {$CellContext`x, 
              0, 1.5}, PlotStyle -> {{Thick, Black}}], 
            
            ParametricPlot3D[{{$CellContext`x, 
               0, (1 - $CellContext`u) \
$CellContext`funcLTC6F32a[$CellContext`x] + $CellContext`u \
$CellContext`funcLTC6F32b[$CellContext`x]}, {$CellContext`x, 
               Sin[$CellContext`\[Theta]Max$$] ((
                  1 - $CellContext`u) \
$CellContext`funcLTC6F32a[$CellContext`x] + $CellContext`u \
$CellContext`funcLTC6F32b[$CellContext`x]), 
               Cos[$CellContext`\[Theta]Max$$] ((
                  1 - $CellContext`u) \
$CellContext`funcLTC6F32a[$CellContext`x] + $CellContext`u \
$CellContext`funcLTC6F32b[$CellContext`x])}}, {$CellContext`x, 0, 
              1}, {$CellContext`u, 0, 1}, PlotPoints -> 20, MaxRecursion -> 0,
              PlotStyle -> {{
                Opacity[1], $CellContext`bcAreaPositiveColor}, {
                Opacity[1], $CellContext`bcAreaPositiveColor}}, Mesh -> None], 
            
            ParametricPlot3D[{$CellContext`xValue$$, 
              Sin[$CellContext`x $CellContext`\[Theta]Max$$] ((
                 
                 1 - $CellContext`u) \
$CellContext`funcLTC6F32a[$CellContext`xValue$$] + $CellContext`u \
$CellContext`funcLTC6F32b[$CellContext`xValue$$]), 
              Cos[$CellContext`x $CellContext`\[Theta]Max$$] ((
                 1 - $CellContext`u) \
$CellContext`funcLTC6F32a[$CellContext`xValue$$] + $CellContext`u \
$CellContext`funcLTC6F32b[$CellContext`xValue$$])}, {$CellContext`x, 0, 
              1}, {$CellContext`u, 0, 1}, PlotPoints -> 20, MaxRecursion -> 0,
              PlotStyle -> {
               Opacity[1], $CellContext`bcB}, Mesh -> None], 
            If[$CellContext`showSurface$$, 
             RevolutionPlot3D[{{$CellContext`x, 0, 
                $CellContext`funcLTC6F32a[$CellContext`x]}, {$CellContext`x, 
                0, 
                $CellContext`funcLTC6F32b[$CellContext`x]}}, {$CellContext`x, 
               0, 1}, {$CellContext`\[Theta], -$CellContext`\[Theta]Max$$, 0},
               RevolutionAxis -> {1, 0, 0}, Mesh -> None, PlotPoints -> 20, 
              MaxRecursion -> 1, PlotStyle -> {
                ControlActive[
                 FaceForm[{
                   Opacity[1], $CellContext`bcSurfaceTop}, {
                   Opacity[1], 
                   Darker[$CellContext`bcAreaPositiveColor]}], 
                 FaceForm[{
                   Opacity[0.5], $CellContext`bcSurfaceTop}, {
                   Opacity[0.5], 
                   Darker[$CellContext`bcAreaPositiveColor]}]], 
                ControlActive[
                 FaceForm[{
                   Opacity[1], 
                   Darker[$CellContext`bcAreaPositiveColor]}, {
                   Opacity[1], $CellContext`bcSurfaceTop}], 
                 FaceForm[{
                   Opacity[0.5], 
                   Darker[$CellContext`bcAreaPositiveColor]}, {
                   Opacity[0.5], $CellContext`bcSurfaceTop}]]}], 
             Graphics3D[{}]], 
            Graphics3D[{
              Text["0", {0, 0, 0}, {0, 1.5}], 
              Text["1", {1, 0, 0}, {0, 1.5}], 
              Text[
              "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", {
               1.5, 0, 0}, {0, -1}], 
              Text[
              "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)", {
               0, 0, 1.5}, {0, -1}], 
              Line[{{-0.5, 0, 0}, {2.5, 0, 0}}], 
              Line[{{0, 0, -0.5}, {0, 0, 12}}], $CellContext`bcR, Thick, 
              Line[{{$CellContext`xValue$$, 0, 
                 $CellContext`funcLTC6F32a[$CellContext`xValue$$]}, \
{$CellContext`xValue$$, 0, 
                 $CellContext`funcLTC6F32b[$CellContext`xValue$$]}}], 
              
              Line[{{$CellContext`xValue$$, 
                 Sin[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F32a[$CellContext`xValue$$], 
                 Cos[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F32a[$CellContext`xValue$$]}, {$CellContext`xValue$$, 
                 Sin[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F32b[$CellContext`xValue$$], 
                 Cos[$CellContext`\[Theta]Max$$] \
$CellContext`funcLTC6F32b[$CellContext`xValue$$]}}]}]}, 
           PlotRange -> {{-0.5, 1.5}, {-1.5, 1.5}, {-1.5, 1.5}}, 
           AxesOrigin -> {0, 0, 0}, Axes -> False, Boxed -> False, Lighting -> 
           "Neutral", ImageSize -> {4 72, 4 72}, ViewPoint -> {10, -10, 5}, 
           BoxRatios -> {1, 1, 1}, BaseStyle -> {"Text", 13}]}}, 
        Alignment -> {Center, Top}], 
      "Specifications" :> {{{$CellContext`xValue$$, 0.5, 
          "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)"}, 0, 1, 0.1, 
         ControlType -> Slider, ImageSize -> Small, ControlPlacement -> 
         1}, {{$CellContext`\[Theta]Max$$, 0.01, "revolve"}, 0.01, 2 Pi, 
         ControlType -> Slider, ImageSize -> Small, ControlPlacement -> 
         2}, {{$CellContext`\[Theta]Max$$, 0.01, ""}, 0.01, 2 Pi, 0.1, 
         ControlType -> Trigger, DefaultDuration -> 10, 
         AppearanceElements -> {
          "PlayPauseButton", "ResetButton", "StepLeftButton", 
           "StepRightButton"}, ControlPlacement -> 
         3}, {{$CellContext`showSurface$$, False, "show solid"}, {
         True, False}, ControlType -> Checkbox, ControlPlacement -> 4}, 
        Row[{
          Manipulate`Place[1], 
          Spacer[10], 
          Manipulate`Place[2], 
          Spacer[10], 
          Manipulate`Place[3], 
          Spacer[10], 
          Manipulate`Place[4]}]}, 
      "Options" :> {
       ControlPlacement -> Top, LabelStyle -> 11, AppearanceElements -> 
        "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{571., {205., 210.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{$CellContext`bcPBS = 
         BaseStyle -> {
           LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
            LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
            Left, FontSize -> 13}, $CellContext`bcFO = {
          RoundingRadius -> 5, FrameStyle -> 
           GrayLevel[0.85]}, $CellContext`funcLTC6F32a[
           Pattern[$CellContext`x, 
            Blank[]]] := $CellContext`x^2, $CellContext`funcLTC6F32b[
           Pattern[$CellContext`x, 
            Blank[]]] := 
         Sqrt[$CellContext`x], $CellContext`bcAreaPositiveColor = 
         RGBColor[0.796, 0.91, 0.855], 
         Attributes[PlotRange] = {ReadProtected}, $CellContext`bcR = 
         RGBColor[0.8, 0, 0.4], $CellContext`bcB = 
         RGBColor[0, 0.4, 0.8], $CellContext`bcSurfaceTop = 
         RGBColor[0.85, 0.6, 0.2]}; {Null, Null}}; Typeset`initDone$$ = True),
    
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1354, 874},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 6.3, Figure 6.32",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"6.3 Volume by Slicing\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 6.3 Volume by Slicing\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 6.3 Volume by Slicing\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
   StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[590, 21, 12140, 238, 432, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature xKznREK3n1dkzI5uIjyr8Rwt *)
