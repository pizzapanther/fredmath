(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 8.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     25368,        527]
NotebookOptionsPosition[     12696,        258]
NotebookOutlinePosition[     25461,        529]
CellTagsIndexPosition[     25418,        526]
WindowTitle->Section 6.3, Figure 6.23
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`k$$ = 2, $CellContext`nValue$$ = 10, 
    Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`k$$], 2, 
       "\!\(\*\nStyleBox[FormBox[\"k\",\n  TraditionalForm],\n\
FontSlant->\"Italic\"]\)"}, 1, 
      Dynamic[$CellContext`nValue$$], 1}, {{
       Hold[$CellContext`nValue$$], 10, 
       "\!\(\*\nStyleBox[FormBox[\"n\",\n  TraditionalForm],\n\
FontSlant->\"Italic\"]\)"}, 2, 20, 1}}, Typeset`size$$ = {324., {174., 181.}},
     Typeset`update$$ = 0, Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`k$136431$$ = 0, $CellContext`nValue$136432$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`k$$ = 2, $CellContext`nValue$$ = 10}, 
      "ControllerVariables" :> {
        Hold[$CellContext`k$$, $CellContext`k$136431$$, 0], 
        Hold[$CellContext`nValue$$, $CellContext`nValue$136432$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, 
      "Body" :> (
       If[$CellContext`k$$ > $CellContext`nValue$$, $CellContext`k$$ = \
$CellContext`nValue$$]; Column[{
          Framed[
           Pane[
            
            Row[{"Volume of \!\(\*\nStyleBox[\"k\",\n\
FontSlant->\"Italic\"]\)th slice", 
              RawBoxes[
               FormBox[
                RowBox[{"\[InvisiblePostfixScriptBase]", "\[TildeTilde]", 
                  RowBox[{
                    RowBox[{"A", "(", 
                    SubscriptBox[
                    OverscriptBox["x", "_"], "k"], ")"}], " ", 
                    "\[CapitalDelta]", "\[VeryThinSpace]", "x"}]}], 
                TraditionalForm]]}]], $CellContext`bcFO], 
          Show[{
            ParametricPlot3D[{$CellContext`x, $CellContext`y, 
              $CellContext`funcLTC6F23[$CellContext`x, $CellContext`y]}, \
{$CellContext`x, 0, 3}, {$CellContext`y, 0, 2}, PlotStyle -> ControlActive[{
                Opacity[1], $CellContext`bcSurfaceTop}, {
                Opacity[0.5], $CellContext`bcSurfaceTop}], 
             PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}, 
             Mesh -> {{($CellContext`k$$ - 1/2) $CellContext`deltaX[
                 0, 3, $CellContext`nValue$$]}, 0}, 
             MeshStyle -> {{Thick, $CellContext`bcR}}, BoxRatios -> {2, 1, 1},
              PlotPoints -> 20, MaxRecursion -> 1], 
            
            ParametricPlot3D[{
             3, $CellContext`y, $CellContext`u $CellContext`funcLTC6F23[
               3, $CellContext`y]}, {$CellContext`u, 0, 1}, {$CellContext`y, 
              0, 2}, PlotStyle -> ControlActive[{
                Opacity[1], $CellContext`bcSurfaceTop}, {
                Opacity[0.5], $CellContext`bcSurfaceTop}], Mesh -> None, 
             MeshStyle -> {{Thick, $CellContext`bcR}}, BoxRatios -> {2, 1, 1},
              PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}], 
            
            ParametricPlot3D[{
             0, $CellContext`y, $CellContext`u $CellContext`funcLTC6F23[
               0, $CellContext`y]}, {$CellContext`u, 0, 1}, {$CellContext`y, 
              0, 2}, PlotStyle -> ControlActive[{
                Opacity[1], $CellContext`bcSurfaceTop}, {
                Opacity[0.5], $CellContext`bcSurfaceTop}], Mesh -> None, 
             MeshStyle -> {{Thick, $CellContext`bcR}}, BoxRatios -> {2, 1, 1},
              PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}], 
            ControlActive[
             Graphics3D[{}], 
             
             ParametricPlot3D[{{
               0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$] + $CellContext`u, \
$CellContext`y, 
                $CellContext`funcLTC6F23[
                0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$], $CellContext`y]}}, \
{$CellContext`u, (-$CellContext`deltaX[0, 3, $CellContext`nValue$$])/
               2, $CellContext`deltaX[0, 3, $CellContext`nValue$$]/
               2}, {$CellContext`y, 0, 2}, PlotStyle -> 
              RGBColor[0.5, 0.5, 0.7], Mesh -> None, 
              PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}, PlotPoints -> 20, 
              MaxRecursion -> 1]], 
            ControlActive[
             Graphics3D[{}], 
             
             ParametricPlot3D[{{
               0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$] + $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$]/
                 2, $CellContext`y, $CellContext`v $CellContext`funcLTC6F23[
                 0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                    0, 3, $CellContext`nValue$$], $CellContext`y]}, {
               0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$] - $CellContext`deltaX[
                 0, 3, $CellContext`nValue$$]/
                2, $CellContext`y, $CellContext`v $CellContext`funcLTC6F23[
                 0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                    0, 3, $CellContext`nValue$$], $CellContext`y]}}, \
{$CellContext`v, 0, 1}, {$CellContext`y, 0, 2}, PlotStyle -> 
              RGBColor[0.5, 0.5, 0.7], Mesh -> None, 
              PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}, PlotPoints -> 20, 
              MaxRecursion -> 1]], 
            Graphics3D[{
              Arrowheads[{0, 0.008}], 
              Text[
              "\!\(\*FormBox[\n RowBox[{\"\[CapitalDelta]\", \"\
\[VeryThinSpace]\", \"x\"}],\n TraditionalForm]\)", {
               0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$], 0.7, 12}], 
              Text[
               Pane[
                Row[{"Cross-sectional", "\n", "area", 
                  RawBoxes[
                   FormBox[
                    RowBox[{"\[InvisiblePostfixScriptBase]", "=", 
                    RowBox[{"A", "(", 
                    SubscriptBox[
                    OverscriptBox["x", "_"], "k"], ")"}]}], 
                    TraditionalForm]]}, " "]], {2.5, 1, 10}, {0, -1}], 
              GrayLevel[0.3], 
              
              Arrow[{{2.5, 1, 10}, {
                0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] + $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$]/2, 0.7, 3}}], 
              
              Line[{{0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] + $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$]/2, 0.7, 
                 $CellContext`funcLTC6F23[
                 0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                    0, 3, $CellContext`nValue$$], 0.7]}, {
                0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] + $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$]/2, 0.7, 13}}], 
              
              Line[{{0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] - $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$]/2, 0.7, 
                 $CellContext`funcLTC6F23[
                 0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                    0, 3, $CellContext`nValue$$], 0.7]}, {
                0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] - $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$]/2, 0.7, 13}}], 
              
              Arrow[{{0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] - 
                 1.2 $CellContext`deltaX[0, 3, $CellContext`nValue$$], 0.7, 
                 12}, {0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] - $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$]/2, 0.7, 12}}], 
              
              Arrow[{{0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] + 
                 1.2 $CellContext`deltaX[0, 3, $CellContext`nValue$$], 0.7, 
                 12}, {0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$] + $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$]/2, 0.7, 12}}], Gray, Dashed, 
              AbsoluteThickness[1.5], 
              
              Line[{{0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$], 0, 0}, {
                0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                   0, 3, $CellContext`nValue$$], 1.3, 0}}], 
              Line[{{0, 0, 0}, {0, 1.3, 0}}], 
              Line[{{3, 0, 0}, {3, 1.3, 0}}]}, 
             PlotRange -> {{0, 3}, {0, 2}, {0, 12}}]}, Lighting -> "Neutral", 
           Boxed -> False, Axes -> {True, False, False}, 
           PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}, 
           ImageSize -> {4.5 72, 4.5 72}, BoxRatios -> {1, 1, 1/GoldenRatio}, 
           AxesOrigin -> {0, 0, 0}, Ticks -> {
             Join[
              
              Table[{0 + $CellContext`i $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$], ""}, {$CellContext`i, 
                0, $CellContext`nValue$$}], {{
               0, "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, {
               3, "\!\(\*\nStyleBox[\"b\",\nFontSlant->\"Italic\"]\)"}, {
               0 + ($CellContext`k$$ - 1/2) $CellContext`deltaX[
                  0, 3, $CellContext`nValue$$], 
                "\!\(\*\nStyleBox[FormBox[SubscriptBox[OverscriptBox[\"x\", \
\"_\"], \"k\"],\n  TraditionalForm],\nFontSlant->\"Italic\"]\)"}}], None, 
             None}, BaseStyle -> {"Text", 13}, 
           PlotRange -> {{0, 3}, {0, 2}, {0, 11.9}}]}, BaseStyle -> "TR", 
         Alignment -> Center]), 
      "Specifications" :> {{{$CellContext`k$$, 2, 
          "\!\(\*\nStyleBox[FormBox[\"k\",\n  TraditionalForm],\n\
FontSlant->\"Italic\"]\)"}, 1, 
         Dynamic[$CellContext`nValue$$], 1, ControlType -> Slider, ImageSize -> 
         Small}, {{$CellContext`nValue$$, 10, 
          "\!\(\*\nStyleBox[FormBox[\"n\",\n  TraditionalForm],\n\
FontSlant->\"Italic\"]\)"}, 2, 20, 1, ControlType -> Slider, ImageSize -> 
         Small}}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, AppearanceElements -> 
        "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{490., {206., 211.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{
        Attributes[RawBoxes] = {ReadProtected}, $CellContext`bcFO = {
          RoundingRadius -> 5, FrameStyle -> 
           GrayLevel[0.85]}, $CellContext`funcLTC6F23[
           Pattern[$CellContext`x, 
            Blank[]], 
           Pattern[$CellContext`y, 
            Blank[]]] := 
         2 ($CellContext`x - 2)^2 + 4 - 
          10 (2 ($CellContext`y - 0.7))^2, $CellContext`bcSurfaceTop = 
         RGBColor[0.85, 0.6, 0.2], 
         Attributes[PlotRange] = {ReadProtected}, $CellContext`deltaX[
           Pattern[$CellContext`xMin, 
            Blank[]], 
           Pattern[$CellContext`xMax, 
            Blank[]], 
           Pattern[$CellContext`n, 
            
            Blank[]]] := ($CellContext`xMax - \
$CellContext`xMin)/$CellContext`n, $CellContext`bcR = 
         RGBColor[0.8, 0, 0.4], $CellContext`i = 1}; {Null, Null}}; 
     Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1354, 874},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 6.3, Figure 6.23",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"6.3 Volume by Slicing\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 6.3 Volume by Slicing\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  6\nApplications of Integration", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 6.3 Volume by Slicing\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
   StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[590, 21, 12102, 235, 434, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature 9LzW8t72fLdp2L51X5sFdfa7 *)
