(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 8.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     24576,        510]
NotebookOptionsPosition[     12175,        248]
NotebookOutlinePosition[     24669,        512]
CellTagsIndexPosition[     24626,        509]
WindowTitle->Section 12.3, Figure 12.44
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`showArea$$ = 
    False, $CellContext`uMagnitude$$ = 1.5, $CellContext`vMagnitude$$ = 
    2, $CellContext`\[Theta]Value$$ = Rational[1, 3] Pi, Typeset`show$$ = 
    True, Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`uMagnitude$$], 1.5, 
       "\[LeftBracketingBar]\!\(\*\nStyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"]\)\[RightBracketingBar]"}, 0.1, 2}, {{
       Hold[$CellContext`vMagnitude$$], 2, 
       "\[LeftBracketingBar]\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"]\)\[RightBracketingBar]"}, 0.1, 2}, {{
       Hold[$CellContext`\[Theta]Value$$], Rational[1, 3] Pi, "\[Theta]"}, 0, 
      Pi, Rational[1, 48] Pi}, {{
       Hold[$CellContext`showArea$$], False, 
       "show \!\(\*FormBox[\n RowBox[{\n  StyleBox[\"u\",\nFontWeight->\"Bold\
\",\nFontSlant->\"Plain\"], \"\[CenterDot]\", \n  StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]}],\n TraditionalForm]\) as a \n\
signed area"}, {True, False}}, {
      Hold[
       Row[{
         Manipulate`Place[1]}]], Manipulate`Dump`ThisIsNotAControl}}, 
    Typeset`size$$ = {360., {156.5, 161.5}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`uMagnitude$188893$$ = 
    0, $CellContext`vMagnitude$188894$$ = 
    0, $CellContext`\[Theta]Value$188895$$ = 
    0, $CellContext`showArea$188896$$ = False}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`showArea$$ = 
        False, $CellContext`uMagnitude$$ = 1.5, $CellContext`vMagnitude$$ = 
        2, $CellContext`\[Theta]Value$$ = Rational[1, 3] Pi}, 
      "ControllerVariables" :> {
        Hold[$CellContext`uMagnitude$$, $CellContext`uMagnitude$188893$$, 0], 
        
        Hold[$CellContext`vMagnitude$$, $CellContext`vMagnitude$188894$$, 0], 
        
        Hold[$CellContext`\[Theta]Value$$, \
$CellContext`\[Theta]Value$188895$$, 0], 
        Hold[$CellContext`showArea$$, $CellContext`showArea$188896$$, False]},
       "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Grid[{{
          Graphics[{Black, 
            Text[
             Framed[
             "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\n   StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[CenterDot]\", \n   \
StyleBox[\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \"=\", \n  \
RowBox[{\n   RowBox[{\"\[LeftBracketingBar]\", \n    StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[RightBracketingBar]\"}], \"\
\[VeryThinSpace]\", \n   RowBox[{\"\[LeftBracketingBar]\", \n    StyleBox[\"v\
\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"], \
\"\[RightBracketingBar]\"}], \"cos\", \" \", \"\[Theta]\"}]}],\n \
TraditionalForm]\)", {RoundingRadius -> 5, FrameStyle -> LightGray}], {
             0, -1}, {0, -1}], 
            Text[
            "\!\(\*FormBox[\n StyleBox[\"v\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"],\n TraditionalForm]\)", $CellContext`vMagnitude$$ {1, 
               0} + {0.1, 0}, {0, 1.1}], 
            Text[
            "\!\(\*FormBox[\n StyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"],\n TraditionalForm]\)", $CellContext`uMagnitude$$ {
                Cos[$CellContext`\[Theta]Value$$], 
                Sin[$CellContext`\[Theta]Value$$]} + 0.1 {
                Cos[$CellContext`\[Theta]Value$$], 
                Sin[$CellContext`\[Theta]Value$$]}, {0, -1.1}], 
            If[$CellContext`showArea$$, {
              Text[
              "\[LeftBracketingBar]\!\(\*FormBox[\n StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"],\n TraditionalForm]\)\
\[RightBracketingBar]", $CellContext`vMagnitude$$ {0, 1/2}, {Sign[
                  Cos[$CellContext`\[Theta]Value$$]] 1.5, 0}], 
              Text[
              "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\"\[LeftBracketingBar]\", \
\n   StyleBox[\"u\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\
\[RightBracketingBar]\"}], \" \", \"cos\", \" \", \"\[Theta]\"}],\n \
TraditionalForm]\)", $CellContext`uMagnitude$$ {
                Cos[$CellContext`\[Theta]Value$$]/2, 0}, {0, 1.5}], Black, 
              Circle[{0, 0}, $CellContext`vMagnitude$$, {0, Pi}], 
              EdgeForm[Black], 
              
              If[$CellContext`\[Theta]Value$$ <= 
               Pi/2, $CellContext`bcAreaPositiveColor, \
$CellContext`bcAreaNegativeColor], 
              Opacity[0.5], 
              
              Rectangle[{0, 
               0}, {$CellContext`uMagnitude$$ 
                Cos[$CellContext`\[Theta]Value$$], \
$CellContext`vMagnitude$$}], 
              Opacity[1]}, Black], Black, 
            Text["\[Theta]", 0.4 {
               Cos[$CellContext`\[Theta]Value$$/2], 
               Sin[$CellContext`\[Theta]Value$$/2]}, {0, 0}], 
            AbsoluteThickness[2], 
            Arrowheads[0.04], $CellContext`bcB, 
            
            Arrow[{{0, 
              0}, $CellContext`vMagnitude$$ {1, 0}}], $CellContext`bcR, 
            Arrow[{{0, 0}, $CellContext`uMagnitude$$ {
                Cos[$CellContext`\[Theta]Value$$], 
                Sin[$CellContext`\[Theta]Value$$]}}], 
            AbsoluteThickness[1], 
            Circle[{0, 0}, 0.3, {0, $CellContext`\[Theta]Value$$}]}, 
           PlotRange -> {{-2.5, 2.5}, {-1, 2.5}}, ImageSize -> 5 72, 
           BaseStyle -> {"Text", 13}]}, {
          Graphics[{Black, 
            Text[
            "\!\(\*FormBox[FormBox[\n  RowBox[{\n   RowBox[{\"\
\[LeftBracketingBar]\", \n    StyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"], \"\[RightBracketingBar]\"}], \"\[VeryThinSpace]\", \n  \
 RowBox[{\"\[LeftBracketingBar]\", \n    StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[RightBracketingBar]\"}]}],\n\
  TraditionalForm],\n TraditionalForm]\)", {$CellContext`uMagnitude$$ \
$CellContext`vMagnitude$$, -1.5}, {-1, -1}], 
            Text[
            "\!\(\*FormBox[\n RowBox[{\"-\", FormBox[\n   RowBox[{\n    \
RowBox[{\"\[LeftBracketingBar]\", \n     StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[RightBracketingBar]\"}], \"\
\[VeryThinSpace]\", \n    RowBox[{\"\[LeftBracketingBar]\", \n     \
StyleBox[\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\
\[RightBracketingBar]\"}]}],\n   TraditionalForm]}],\n TraditionalForm]\)", \
{(-$CellContext`uMagnitude$$) $CellContext`vMagnitude$$, -1.5}, {1, -1}], 
            Text[
            "\!\(\*FormBox[\"0\",\n TraditionalForm]\)", {0, -1.5}, {0, -1}], 
            
            Text["\[VerticalSeparator]", {0, 0}, {0, 0}], 
            Text[
             Which[
              Inequality[
              0, LessEqual, $CellContext`\[Theta]Value$$, Less, Pi/2], 
              "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\n   StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[CenterDot]\", \n   \
StyleBox[\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \">\", \
\"0\"}],\n TraditionalForm]\)", $CellContext`\[Theta]Value$$ == Pi/2, 
              "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\n   StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[CenterDot]\", \n   \
StyleBox[\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \"=\", \
\"0\"}],\n TraditionalForm]\)", 
              Inequality[
              Pi/2, Less, $CellContext`\[Theta]Value$$, LessEqual, Pi], 
              "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\n   StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[CenterDot]\", \n   \
StyleBox[\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \"<\", \
\"0\"}],\n TraditionalForm]\)"], {($CellContext`uMagnitude$$ \
$CellContext`vMagnitude$$) Cos[$CellContext`\[Theta]Value$$], 0.5}, {
             0, -1}], $CellContext`bcG, 
            AbsoluteThickness[3], 
            
            Line[{{0, 
              0}, {$CellContext`uMagnitude$$ $CellContext`vMagnitude$$, 0}}], 
            
            Text[
             Style[
             "]", Bold, 
              16], {$CellContext`uMagnitude$$ $CellContext`vMagnitude$$, 0}, {
             0, 0}], Orange, 
            Text[
             Style[
             "[", Bold, 
              16], {(-$CellContext`uMagnitude$$) $CellContext`vMagnitude$$, 
              0}, {0, 0}], 
            
            Line[{{(-$CellContext`uMagnitude$$) $CellContext`vMagnitude$$, 
               0}, {0, 0}}], 
            Which[
             Inequality[
             0, LessEqual, $CellContext`\[Theta]Value$$, Less, Pi/
              2], $CellContext`bcG, $CellContext`\[Theta]Value$$ == Pi/2, 
             Black, 
             Inequality[
             Pi/2, Less, $CellContext`\[Theta]Value$$, LessEqual, Pi], 
             Orange], 
            Text[
            "\[FilledDownTriangle]", {($CellContext`uMagnitude$$ \
$CellContext`vMagnitude$$) Cos[$CellContext`\[Theta]Value$$], 0}, {0, -1}]}, 
           PlotRange -> {{-8, 8}, {-1.5, 1.5}}, AspectRatio -> 2/12, 
           ImageSize -> 5 72, Axes -> {True, False}, 
           BaseStyle -> {"Text", 13}, Ticks -> None, AxesStyle -> 
           Arrowheads[{-0.03, 0.03}]]}}], 
      "Specifications" :> {{{$CellContext`uMagnitude$$, 1.5, 
          "\[LeftBracketingBar]\!\(\*\nStyleBox[\"u\",\nFontWeight->\"Bold\",\
\nFontSlant->\"Plain\"]\)\[RightBracketingBar]"}, 0.1, 2, ControlType -> 
         Slider, ImageSize -> 
         Small}, {{$CellContext`vMagnitude$$, 2, 
          "\[LeftBracketingBar]\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\",\
\nFontSlant->\"Plain\"]\)\[RightBracketingBar]"}, 0.1, 2, ControlType -> 
         Slider, ImageSize -> 
         Small}, {{$CellContext`\[Theta]Value$$, Rational[1, 3] Pi, 
          "\[Theta]"}, 0, Pi, Rational[1, 48] Pi, ControlType -> Slider, 
         ImageSize -> Small}, 
        Delimiter, {{$CellContext`showArea$$, False, 
          "show \!\(\*FormBox[\n RowBox[{\n  StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[CenterDot]\", \n  StyleBox[\
\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}],\n TraditionalForm]\) \
as a \nsigned area"}, {True, False}, ControlType -> Checkbox, 
         ControlPlacement -> 1}, 
        Row[{
          Manipulate`Place[1]}]}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton", FrameLabel -> 
        Style["FIGURE 12.44", "FigureFont", Smaller]}, "DefaultOptions" :> {}],
     ImageSizeCache->{533., {196., 201.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({$CellContext`bcAreaPositiveColor = 
       RGBColor[0.796, 0.91, 0.855], $CellContext`bcAreaNegativeColor = 
       RGBColor[0.976, 0.871, 0.812], $CellContext`bcB = 
       RGBColor[0, 0.4, 0.8], $CellContext`bcR = RGBColor[0.8, 0, 0.4], 
       Attributes[PlotRange] = {ReadProtected}, $CellContext`bcG = 
       RGBColor[0, 0.6, 0.4]}; Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1309, 895},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 12.3, Figure 12.44",
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"12.3 Dot Products\"", "SR", FontSize -> 12, Bold, StripOnInput -> 
             False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.3 Dot Products\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.3 Dot Products\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->"Default.nb"
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[592, 21, 11579, 225, 414, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature nKNdp5syjCCl9JkSqYM9TpD@ *)
