(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 9.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     21404,        454]
NotebookOptionsPosition[      9004,        192]
NotebookOutlinePosition[     21498,        456]
CellTagsIndexPosition[     21455,        453]
WindowTitle->Section 12.3, Figure 12.48
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`uVector$$ = {
    1.8, 4}, $CellContext`vVector$$ = {3, 0}, Typeset`show$$ = True, 
    Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`uVector$$], {1.8, 4}}, {-5, -5}, {5, 5}}, {{
       Hold[$CellContext`vVector$$], {3, 0}}, {-5, -5}, {5, 5}}, {
      Hold[
       Pane[
       "Drag the symbols \[CircleTimes] to change \!\(\*\nStyleBox[\"u\",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)and\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\"]\)\!\(\*\
\nStyleBox[\".\",\nFontWeight->\"Bold\"]\)", {110, Automatic}, 
        BaseStyle -> {
         LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
          LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
          Left, FontSize -> 11}]], Manipulate`Dump`ThisIsNotAControl}}, 
    Typeset`size$$ = {288., {141., 147.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`uVector$189003$$ = {0, 
    0}, $CellContext`vVector$189004$$ = {0, 0}}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`uVector$$ = {
         1.8, 4}, $CellContext`vVector$$ = {3, 0}}, 
      "ControllerVariables" :> {
        Hold[$CellContext`uVector$$, $CellContext`uVector$189003$$, {0, 0}], 
        Hold[$CellContext`vVector$$, $CellContext`vVector$189004$$, {0, 0}]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Graphics[{
         AbsoluteThickness[0.5], 
         Line[{10 {
             Cos[
              Arg[
               Dot[$CellContext`vVector$$, {1, I}]]], 
             Sin[
              Arg[
               Dot[$CellContext`vVector$$, {1, I}]]]}, 10 {
             Cos[Arg[
                Dot[$CellContext`vVector$$, {1, I}]] + Pi], 
             Sin[Arg[
                Dot[$CellContext`vVector$$, {1, I}]] + Pi]}}], 
         AbsoluteThickness[4], 
         Arrowheads[0.05], 
         GrayLevel[0.6], 
         Arrow[{{0, 
           0}, (Dot[$CellContext`uVector$$, $CellContext`vVector$$]/
            Dot[$CellContext`vVector$$, $CellContext`vVector$$]) \
$CellContext`vVector$$}], 
         AbsoluteThickness[2], 
         AbsolutePointSize[6], 
         Arrowheads[0.04], Black, 
         Point[{0, 0}], 
         Text[
         "\!\(\*\nStyleBox[\"O\",\nFontSlant->\"Italic\"]\)", {0, 0}, {1.5, 
          1.5}], 
         Text[
         "\[Theta]", ($CellContext`uVector$$/
            Norm[$CellContext`uVector$$] + $CellContext`vVector$$/
            Norm[$CellContext`vVector$$])/
          Norm[$CellContext`uVector$$/
            Norm[$CellContext`uVector$$] + $CellContext`vVector$$/
            Norm[$CellContext`vVector$$]], {0, 0}], 
         Text[
         "\!\(\*\nStyleBox[FormBox[\n  StyleBox[\"u\",\nFontWeight->\"Bold\",\
\nFontSlant->\"Plain\"],\n  TraditionalForm],\nFontSlant->\"Italic\"]\)", \
$CellContext`uVector$$ + 
          0.5 ($CellContext`uVector$$/Norm[$CellContext`uVector$$]), {0, 0}], 
         
         Text[
          Pane[
          "\!\(\*\nStyleBox[FormBox[\n  StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"],\n  TraditionalForm],\n\
FontSlant->\"Italic\"]\)", 
           BaseStyle -> {Background -> White}], $CellContext`vVector$$ + 
          0.5 ($CellContext`vVector$$/Norm[$CellContext`vVector$$]), {0, 0}], 
         
         Text[
         "\!\(\*\nStyleBox[\"P\",\nFontSlant->\"Italic\"]\)", (
           Dot[$CellContext`uVector$$, $CellContext`vVector$$]/
           Dot[$CellContext`vVector$$, $CellContext`vVector$$]) \
$CellContext`vVector$$, {0, 1.5}], $CellContext`bcR, 
         Arrow[{{0, 0}, $CellContext`uVector$$}], $CellContext`bcB, 
         Arrow[{{0, 0}, $CellContext`vVector$$}], 
         AbsoluteThickness[1], $CellContext`bcR, 
         Circle[{0, 0}, 0.75, 
          Apply[Which[0 <= Abs[# - #2] <= Pi, 
            Sort[{#, #2}], Abs[# - #2] > Pi, 
            Sort[{
              If[#2 < 0, #2 + 2 Pi, #2], 
              If[# < 0, # + 2 Pi, #]}]]& , {
            Arg[
             Dot[$CellContext`uVector$$, {1, I}]], 
            Arg[
             Dot[$CellContext`vVector$$, {1, I}]]}]], Dashed, Black, 
         Line[{$CellContext`uVector$$, (
            Dot[$CellContext`uVector$$, $CellContext`vVector$$]/
            Dot[$CellContext`vVector$$, $CellContext`vVector$$]) \
$CellContext`vVector$$}], 
         Dashing[{}], 
         Text[
          Framed[
           Pane[
           "\!\(\*FormBox[GridBox[{\n   {\n    RowBox[{\n     \
RowBox[{SubscriptBox[\"proj\", \n       StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]], \n      StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \"=\", \n     \
RowBox[{OverscriptBox[\n       RowBox[{\"O\", \"\[VeryThinSpace]\", \"P\"}], \
\"\[RightVector]\"], \"=\", \n      RowBox[{\n       RowBox[{\"\
\[LeftBracketingBar]\", \n        StyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"], \"\[RightBracketingBar]\"}], \"cos\", \" \", \
\"\[Theta]\", \" \", \n       RowBox[{\"(\", FractionBox[\n         StyleBox[\
\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"], \n         RowBox[{\"\
\[LeftBracketingBar]\", \n          StyleBox[\"v\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"], \"\[RightBracketingBar]\"}]], \")\"}]}]}]}]},\n   {\n  \
  RowBox[{\n     RowBox[{SubscriptBox[\"scal\", \n       StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]], \n      StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \n     StyleBox[\"=\",\n\
FontSlant->\"Plain\"], \n     RowBox[{\n      \
RowBox[{\"\[LeftBracketingBar]\", \n       StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"\[RightBracketingBar]\"}], \
\"cos\", \" \", \"\[Theta]\", \" \"}]}]}\n  },\n\
GridBoxAlignment->{\"Columns\" -> {{\"=\"}}}],\n TraditionalForm]\)", \
$CellContext`bcPBS], $CellContext`bcFO, Background -> White], {0, -5}, {
          0, -1.1}]}, PlotRange -> {{-5, 5}, {-5, 5}}, ImageSize -> 4 72, 
        BaseStyle -> {"Text", 13}], 
      "Specifications" :> {{{$CellContext`uVector$$, {1.8, 4}}, {-5, -5}, {5, 
         5}, ControlType -> Locator, Appearance -> 
         "\[CircleTimes]"}, {{$CellContext`vVector$$, {3, 0}}, {-5, -5}, {5, 
         5}, ControlType -> Locator, Appearance -> "\[CircleTimes]"}, 
        Pane[
        "Drag the symbols \[CircleTimes] to change \!\(\*\nStyleBox[\"u\",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)and\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\"]\)\!\(\*\
\nStyleBox[\".\",\nFontWeight->\"Bold\"]\)", {110, Automatic}, 
         BaseStyle -> {
          LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
           LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
           Left, FontSize -> 11}]}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton", FrameLabel -> 
        Style["FIGURE 12.48", "FigureFont", Smaller]}, "DefaultOptions" :> {}],
     ImageSizeCache->{457., {184., 189.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({$CellContext`bcR = 
       RGBColor[0.8, 0, 0.4], $CellContext`bcB = 
       RGBColor[0, 0.4, 0.8], $CellContext`bcPBS = 
       BaseStyle -> {
         LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
          LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
          Left, FontSize -> 13}, $CellContext`bcFO = {
        RoundingRadius -> 5, FrameStyle -> GrayLevel[0.85]}, 
       Attributes[PlotRange] = {ReadProtected}}; Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1309, 895},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 12.3, Figure 12.48",
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"12.3 Dot Products\"", "SR", FontSize -> 12, Bold, StripOnInput -> 
             False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.3 Dot Products\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.3 Dot Products\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"9.0 for Microsoft Windows (64-bit) (November 20, 2012)",
StyleDefinitions->"Default.nb"
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[592, 21, 8408, 169, 390, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature uIz3kZpW9m1aMIkGR7fEQUxD *)
