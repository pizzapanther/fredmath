(* Content-type: application/mathematica *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 7.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       145,          7]
NotebookDataLength[     20832,        435]
NotebookOptionsPosition[      8127,        166]
NotebookOutlinePosition[     20919,        437]
CellTagsIndexPosition[     20876,        434]
WindowTitle->Section 12.1, Figure 12.14
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`uVector$$ = {6, 
    2}, $CellContext`vVector$$ = {2, 5}, Typeset`show$$ = True, 
    Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`uVector$$], {6, 2}}, Automatic}, {{
       Hold[$CellContext`vVector$$], {2, 5}}, Automatic}, {
      Hold[
       Pane[
       "Drag the symbols \[CirclePlus] to change \!\(\*\nStyleBox[\"u\",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)and\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\"]\)\!\(\*\
\nStyleBox[\".\",\nFontWeight->\"Bold\"]\)", {110, Automatic}, 
        BaseStyle -> {
         LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
          LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
          Left, FontSize -> 11}]], Manipulate`Dump`ThisIsNotAControl}}, 
    Typeset`size$$ = {360., {179., 186.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = False}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`uVector$$ = {6, 
         2}, $CellContext`vVector$$ = {2, 5}}, "ControllerVariables" :> {}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Graphics[{
         AbsoluteThickness[2], 
         AbsolutePointSize[6], $CellContext`bcB, 
         Arrow[{{0, 0}, $CellContext`vVector$$}], Black, 
         Arrow[{{0, 0}, $CellContext`uVector$$}], $CellContext`bcR, 
         Arrow[{{0, 0}, $CellContext`uVector$$ + $CellContext`vVector$$}], 
         Black, 
         Text[
         "\!\(\*FormBox[SubscriptBox[\"u\", \"1\"],\n TraditionalForm]\)", \
$CellContext`uVector$$ {1, 0}, {0, 2}], 
         Text[
         "\!\(\*FormBox[SubscriptBox[\"u\", \"2\"],\n TraditionalForm]\)", \
$CellContext`uVector$$ {0, 1}, {2, 0}], 
         Text[
         "\!\(\*FormBox[SubscriptBox[\"v\", \"1\"],\n TraditionalForm]\)", \
$CellContext`vVector$$ {1, 0}, {0, 2}], 
         Text[
         "\!\(\*FormBox[SubscriptBox[\"v\", \"2\"],\n TraditionalForm]\)", \
$CellContext`vVector$$ {0, 1}, {2, 0}], 
         Text[
         "\!\(\*FormBox[\n RowBox[{SubscriptBox[\"u\", \"1\"], \"+\", \
SubscriptBox[\"v\", \"1\"]}],\n TraditionalForm]\)", ($CellContext`uVector$$ + \
$CellContext`vVector$$) {1, 0}, {0, 2}], 
         Text[
         "\!\(\*FormBox[\n RowBox[{SubscriptBox[\"u\", \"2\"], \"+\", \
SubscriptBox[\"v\", \"2\"]}],\n TraditionalForm]\)", ($CellContext`uVector$$ + \
$CellContext`vVector$$) {0, 1}, {1.3, 0}], 
         Text[
         "\!\(\*FormBox[\n StyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"],\n TraditionalForm]\)", $CellContext`uVector$$ {
           1/2, 1/2}, {0, 2}], 
         Text[
         "\!\(\*FormBox[\n StyleBox[\"v\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"],\n TraditionalForm]\)", $CellContext`vVector$$ {
           1/2, 1/2}, {2, -1}], 
         Text[
         "\!\(\*FormBox[\n RowBox[{\n  StyleBox[\"u\",\nFontWeight->\"Bold\",\
\nFontSlant->\"Plain\"], \"+\", \n  StyleBox[\"v\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"]}],\n TraditionalForm]\)", ($CellContext`uVector$$ + \
$CellContext`vVector$$) {1/2, 1/2}, {1, -2}], 
         Text[
         "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\n   \
RowBox[{SubscriptBox[\"u\", \"1\"], \"+\", SubscriptBox[\"v\", \"1\"]}], \
\",\", \n   RowBox[{SubscriptBox[\"u\", \"2\"], \"+\", SubscriptBox[\"v\", \
\"2\"]}]}], \")\"}],\n TraditionalForm]\)", $CellContext`uVector$$ + \
$CellContext`vVector$$, {0, -2}], 
         Text[
          Framed[
           Pane[
           "\!\(\*FormBox[\n RowBox[{\n  RowBox[{\n   StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"], \"+\", \n   StyleBox[\"v\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]}], \"=\", \n  RowBox[{\"\
\[LeftAngleBracket]\", \n   RowBox[{\n    RowBox[{SubscriptBox[\"u\", \"1\"], \
\"+\", SubscriptBox[\"v\", \"1\"]}], \",\", \n    RowBox[{SubscriptBox[\"u\", \
\"2\"], \"+\", SubscriptBox[\"v\", \"2\"]}]}], \"\[RightAngleBracket]\"}]}],\n\
 TraditionalForm]\) by the Parallelogram Rule", {
            180, Automatic}, $CellContext`bcPBS], $CellContext`bcFO, 
           Background -> White], {5, 10}, {0, 1}], $CellContext`bcR, 
         AbsoluteThickness[0.5], Dashed, 
         Line[{($CellContext`uVector$$ + $CellContext`vVector$$) {0, 
            1}, $CellContext`uVector$$ + $CellContext`vVector$$, \
($CellContext`uVector$$ + $CellContext`vVector$$) {1, 0}}], Black, 
         Line[{$CellContext`vVector$$, $CellContext`uVector$$ + \
$CellContext`vVector$$, $CellContext`uVector$$}], $CellContext`bcR, 
         Point[$CellContext`uVector$$ + $CellContext`vVector$$]}, 
        PlotRange -> {{-2, 10}, {-2, 10}}, Axes -> True, AxesStyle -> 
        Arrowheads[{0, 0.03}], 
        AxesLabel -> {
         "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", 
          "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)"}, Ticks -> {{{
            Part[$CellContext`uVector$$, 1], ""}, {
            Part[$CellContext`vVector$$, 1], ""}, {
            Part[$CellContext`uVector$$ + $CellContext`vVector$$, 1], ""}}, {{
           
            Part[$CellContext`uVector$$, 2], ""}, {
            Part[$CellContext`vVector$$, 2], ""}, {
            Part[$CellContext`uVector$$ + $CellContext`vVector$$, 2], ""}}}, 
        ImageSize -> 5 72, BaseStyle -> {"Text", 14}], 
      "Specifications" :> {{{$CellContext`uVector$$, {6, 2}}, Automatic, 
         ControlType -> Locator}, {{$CellContext`vVector$$, {2, 5}}, 
         Automatic, ControlType -> Locator}, 
        Pane[
        "Drag the symbols \[CirclePlus] to change \!\(\*\nStyleBox[\"u\",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)and\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\"]\)\!\(\*\
\nStyleBox[\".\",\nFontWeight->\"Bold\"]\)", {110, Automatic}, 
         BaseStyle -> {
          LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
           LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
           Left, FontSize -> 11}]}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{522., {211.84375, 217.15625}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({$CellContext`bcB = 
       RGBColor[0, 0.4, 0.8], $CellContext`bcR = 
       RGBColor[0.8, 0, 0.4], $CellContext`bcPBS = 
       BaseStyle -> {
         LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
          LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
          Left, FontSize -> 13}, $CellContext`bcFO = {
        RoundingRadius -> 5, FrameStyle -> GrayLevel[0.85]}, 
       Attributes[PlotRange] = {ReadProtected}}; Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{800, 600},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 12.1, Figure 12.14",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"12.1 Vectors in the Plane\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.1 Vectors in the Plane\n", "SR", FontSize -> 11]}],
     CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.1 Vectors in the Plane\n", "SR", FontSize -> 11]}],
     CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"7.0 for Mac OS X x86 (32-bit) (February 18, 2009)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "7.0 for Mac OS X x86 (32-bit) (February 18, 2009)", 
  StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[585, 21, 7538, 143, 447, "Output"]
}
]
*)

(* End of internal cache information *)
(* NotebookSignature mINoKs9oCPkSDI52NMXOFkyx *)
