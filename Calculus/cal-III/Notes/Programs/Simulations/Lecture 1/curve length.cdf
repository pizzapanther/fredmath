(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 9.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     29478,        623]
NotebookOptionsPosition[     17065,        361]
NotebookOutlinePosition[     29571,        625]
CellTagsIndexPosition[     29528,        622]
WindowTitle->Section 12.8, Figure 12.95
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`aValue$$ = 0.2, $CellContext`bValue$$ = 
    2., $CellContext`displayChoices$$ = {1}, $CellContext`kValue$$ = 
    1, $CellContext`nValue$$ = 2, $CellContext`showTAxis$$ = False, 
    Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`nValue$$], 2, 
       "\!\(\*\nStyleBox[\"n\",\nFontSlant->\"Italic\"]\)"}, 2, 10, 1}, {{
       Hold[$CellContext`kValue$$], 1, 
       "\!\(\*\nStyleBox[\"k\",\nFontSlant->\"Italic\"]\)"}, 1, 
      Dynamic[$CellContext`nValue$$], 1}, {{
       Hold[$CellContext`displayChoices$$], {1}, ""}, {
      1 -> "curve", 2 -> "approximation", 3 -> "triangles"}}, {{
       Hold[$CellContext`showTAxis$$], False, 
       "show \!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\) axis"}, {
      True, False}}, {
      Hold[
       Row[{
         Manipulate`Place[1]}]], Manipulate`Dump`ThisIsNotAControl}, {{
       Hold[$CellContext`aValue$$], 0.2, 
       "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, 0.1, 1, 0.01}, {{
      
       Hold[$CellContext`bValue$$], 2., 
       "\!\(\*\nStyleBox[\"b\",\nFontSlant->\"Italic\"]\)"}, 1.4, 2.4, 0.01}},
     Typeset`size$$ = {432., {166., 172.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`nValue$191863$$ = 0, $CellContext`kValue$191864$$ = 
    0, $CellContext`displayChoices$191865$$ = 
    False, $CellContext`showTAxis$191866$$ = 
    False, $CellContext`aValue$191867$$ = 0, $CellContext`bValue$191868$$ = 
    0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`aValue$$ = 0.2, $CellContext`bValue$$ = 
        2., $CellContext`displayChoices$$ = {1}, $CellContext`kValue$$ = 
        1, $CellContext`nValue$$ = 2, $CellContext`showTAxis$$ = False}, 
      "ControllerVariables" :> {
        Hold[$CellContext`nValue$$, $CellContext`nValue$191863$$, 0], 
        Hold[$CellContext`kValue$$, $CellContext`kValue$191864$$, 0], 
        Hold[$CellContext`displayChoices$$, \
$CellContext`displayChoices$191865$$, False], 
        Hold[$CellContext`showTAxis$$, $CellContext`showTAxis$191866$$, 
         False], 
        Hold[$CellContext`aValue$$, $CellContext`aValue$191867$$, 0], 
        Hold[$CellContext`bValue$$, $CellContext`bValue$191868$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, 
      "Body" :> (
       If[$CellContext`kValue$$ > $CellContext`nValue$$, \
$CellContext`kValue$$ = $CellContext`nValue$$]; Show[{
          ParametricPlot[
           $CellContext`funcLTC12F95[$CellContext`t], {$CellContext`t, 0, 
            2.4}, PlotStyle -> {
             If[
              MemberQ[$CellContext`displayChoices$$, 1], {Thick, Black}, 
              None]}], 
          Graphics[{Black, 
            AbsoluteThickness[0.5], 
            Table[
             
             Line[{$CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] {1, 0}, 
               $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)]}], {$CellContext`i, 
              0, $CellContext`nValue$$}], 
            Table[
             
             Line[{$CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] {0, 1}, 
               $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)]}], {$CellContext`i, \
$CellContext`kValue$$ - 1, $CellContext`kValue$$}], 
            AbsoluteThickness[1], 
            If[
             MemberQ[$CellContext`displayChoices$$, 3], {
              Lighter[Brown, 0.8], 
              Opacity[0.5], 
              Polygon[{
                $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 
                   1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], \
$CellContext`funcLTC12F95[$CellContext`aValue$$ + ($CellContext`kValue$$ - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] {0, 
                  1} + $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`kValue$$ (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] {1, 0}, 
                $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`kValue$$ (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)]}], 
              Opacity[1], $CellContext`bcR, 
              AbsoluteThickness[0.5], 
              Arrowheads[0.02], 
              Arrow[{{
                 Part[
                  $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                    2) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 1], 3.3}, {
                 Part[
                  $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                    2) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 1], 
                 
                 Part[($CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] + \
$CellContext`funcLTC12F95[$CellContext`aValue$$ + $CellContext`kValue$$ \
(($CellContext`bValue$$ - $CellContext`aValue$$)/$CellContext`nValue$$)])/2, 
                  2]}}], Black, 
              Text[
               RawBoxes[
                FormBox[
                 SqrtBox[
                  RowBox[{
                    SuperscriptBox[
                    RowBox[{"(", 
                    RowBox[{"\[CapitalDelta]", "\[VeryThinSpace]", 
                    SubscriptBox["x", $CellContext`kValue$$]}], ")"}], "2"], 
                    "+", 
                    SuperscriptBox[
                    RowBox[{"(", 
                    RowBox[{"\[CapitalDelta]", "\[VeryThinSpace]", 
                    SubscriptBox["y", $CellContext`kValue$$]}], ")"}], 
                    "2"]}]], TraditionalForm]], {
                Part[
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                    2) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 1], 3.4}, {
               0, -1}], $CellContext`bcR, 
              Table[
               Line[{
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`i - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], \
$CellContext`funcLTC12F95[$CellContext`aValue$$ + ($CellContext`i - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] {0, 
                   1} + $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)] {1, 0}, 
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`i - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)]}], {$CellContext`i, 
                1, $CellContext`nValue$$}]}, Black], 
            If[
             MemberQ[$CellContext`displayChoices$$, 2], {$CellContext`bcR, 
              AbsoluteThickness[2], 
              Table[
               Line[{
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`i - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)]}], {$CellContext`i, 
                1, $CellContext`nValue$$}]}, Black], $CellContext`bcB, 
            AbsolutePointSize[7], 
            Point[
             Table[
              $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], {$CellContext`i, 
               0, $CellContext`nValue$$}]], 
            AbsoluteThickness[1], 
            If[$CellContext`showTAxis$$, {$CellContext`bcG, 
              Arrowheads[0.025], 
              Arrow[{{0.1, -2}, {4, -2}}], 
              AbsoluteThickness[0.5], $CellContext`bcG, 
              Table[
               
               Line[{{$CellContext`aValue$$ + $CellContext`i \
(($CellContext`bValue$$ - $CellContext`aValue$$)/$CellContext`nValue$$), -2}, {
                  Part[
                   $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 1], 0}}], {$CellContext`i, 
                0, $CellContext`nValue$$}], 
              AbsoluteThickness[1], 
              Table[
               
               Line[{{$CellContext`aValue$$ + $CellContext`i \
(($CellContext`bValue$$ - $CellContext`aValue$$)/$CellContext`nValue$$), \
-2.05}, {$CellContext`aValue$$ + $CellContext`i (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$), -1.95}}], {$CellContext`i, 
                0, $CellContext`nValue$$}], Black, 
              Text[
               Row[{
                 Subscript[
                 "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)", 0], 
                 "\!\(\*FormBox[\n RowBox[{\"\[InvisiblePostfixScriptBase]\", \
\"=\", \"a\"}],\n TraditionalForm]\)"}], {$CellContext`aValue$$, -2}, {
               0, 1.5}], 
              Text[
               Row[{
                 Subscript[
                 "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)", \
$CellContext`nValue$$], 
                 "\!\(\*FormBox[\n RowBox[{\"\[InvisiblePostfixScriptBase]\", \
\"=\", \"b\"}],\n TraditionalForm]\)"}], {$CellContext`bValue$$, -2}, {
               0, 1.5}], 
              Table[
               Text[
                Subscript[
                "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)", \
$CellContext`i], {$CellContext`aValue$$ + $CellContext`i \
(($CellContext`bValue$$ - $CellContext`aValue$$)/$CellContext`nValue$$), -2}, \
{0, 1.5}], {$CellContext`i, 1, $CellContext`nValue$$ - 1}], 
              Text[
              "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)", {
               4, -2}, {-2, 0}], 
              Text[
               Subscript[
               "\!\(\*FormBox[\n RowBox[{\"\[CapitalDelta]\", \"\
\[VeryThinSpace]\", \"t\"}],\n TraditionalForm]\)", $CellContext`kValue$$], \
{$CellContext`aValue$$ + ($CellContext`kValue$$ - 1/
                  2) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$), -2}, {-0.5, -1.5}]}, Black], 
            Black, 
            Text[
             Subscript[
             "\!\(\*FormBox[\n RowBox[{\"\[CapitalDelta]\", \
\"\[VeryThinSpace]\", \"x\"}],\n TraditionalForm]\)", $CellContext`kValue$$], {
              Part[
               $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 1/
                  2) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 1], 0}, {0, -1.5}], 
            Text[
             Subscript[
             "\!\(\*FormBox[\n RowBox[{\"\[CapitalDelta]\", \
\"\[VeryThinSpace]\", \"y\"}],\n TraditionalForm]\)", $CellContext`kValue$$], \
{0, (Part[
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
$CellContext`kValue$$ (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 2] + Part[
                 $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
($CellContext`kValue$$ - 
                    1) (($CellContext`bValue$$ - \
$CellContext`aValue$$)/$CellContext`nValue$$)], 2])/2}, {-1.5, 0}], 
            EdgeForm[White], White, 
            Rectangle[{-0.01, -3}, {0.01, -0.5}]}]}, 
         PlotRange -> {{0, 4}, {-2.5, 4}}, AxesOrigin -> {0, 0}, AspectRatio -> 
         0.8, Ticks -> {
           Table[{
             Part[
              $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
(($CellContext`bValue$$ - $CellContext`aValue$$)/$CellContext`nValue$$) \
$CellContext`i], 1], 
             Pane[
              Row[{"\!\(\*\nStyleBox[\"f\",\nFontSlant->\"Italic\"]\)", "(", 
                Subscript[
                "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)", \
$CellContext`i], ")"}], BaseStyle -> {Background -> White}]}, {$CellContext`i,
              0, $CellContext`nValue$$}], 
           Table[{
             Part[
              $CellContext`funcLTC12F95[$CellContext`aValue$$ + \
(($CellContext`bValue$$ - $CellContext`aValue$$)/$CellContext`nValue$$) \
$CellContext`i], 2], 
             Row[{"\!\(\*\nStyleBox[\"g\",\nFontSlant->\"Italic\"]\)", "(", 
               Subscript[
               "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)", \
$CellContext`i], ")"}]}, {$CellContext`i, $CellContext`kValue$$ - 
             1, $CellContext`kValue$$}]}, ImageSize -> 6 72, AxesStyle -> 
         Arrowheads[{0, 0.025}], 
         AxesLabel -> {
          "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", 
           "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)"}, 
         BaseStyle -> {"Text", 13}, Method -> {"AxesInFront" -> False}]), 
      "Specifications" :> {{{$CellContext`nValue$$, 2, 
          "\!\(\*\nStyleBox[\"n\",\nFontSlant->\"Italic\"]\)"}, 2, 10, 1, 
         ControlType -> Slider, ImageSize -> 
         Small}, {{$CellContext`kValue$$, 1, 
          "\!\(\*\nStyleBox[\"k\",\nFontSlant->\"Italic\"]\)"}, 1, 
         Dynamic[$CellContext`nValue$$], 1, ControlType -> Slider, ImageSize -> 
         Small}, Delimiter, {{$CellContext`displayChoices$$, {1}, ""}, {
         1 -> "curve", 2 -> "approximation", 3 -> "triangles"}, ControlType -> 
         CheckboxBar, Appearance -> "Vertical"}, 
        Delimiter, {{$CellContext`showTAxis$$, False, 
          "show \!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\) axis"}, {
         True, False}, ControlType -> Checkbox, ControlPlacement -> 1}, 
        Row[{
          Manipulate`Place[1]}], {{$CellContext`aValue$$, 0.2, 
          "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, 0.1, 1, 0.01, 
         ControlType -> Slider, ImageSize -> Small, Enabled :> 
         Dynamic[$CellContext`showTAxis$$]}, {{$CellContext`bValue$$, 2., 
          "\!\(\*\nStyleBox[\"b\",\nFontSlant->\"Italic\"]\)"}, 1.4, 2.4, 
         0.01, ControlType -> Slider, ImageSize -> Small, Enabled :> 
         Dynamic[$CellContext`showTAxis$$]}}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton", FrameLabel -> 
        Style["FIGURE 12.95", "FigureFont", Smaller]}, "DefaultOptions" :> {}],
     ImageSizeCache->{599., {206., 211.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{$CellContext`funcLTC12F95[
           Pattern[$CellContext`t, 
            Blank[]]] := {
          4 Sin[$CellContext`t/2], 3 - 
           0.5 (4 Sin[$CellContext`t/2] - 2)^2}, $CellContext`t[
           Pattern[$CellContext`df, 
            Blank[]]] := PDF[
           
           StudentTDistribution[$CellContext`df], $CellContext`x], \
$CellContext`i = 1, $CellContext`bcR = RGBColor[0.8, 0, 0.4], 
         Attributes[RawBoxes] = {ReadProtected}, $CellContext`bcB = 
         RGBColor[0, 0.4, 0.8], $CellContext`bcG = RGBColor[0, 0.6, 0.4], 
         Attributes[Subscript] = {NHoldRest}, 
         Attributes[PlotRange] = {ReadProtected}}; {Null}}; 
     Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1309, 895},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 12.8, Figure 12.95",
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"12.8 Length of Curves\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.8 Length of Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.8 Length of Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"9.0 for Microsoft Windows (64-bit) (November 20, 2012)",
StyleDefinitions->"Default.nb"
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[592, 21, 16469, 338, 434, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature lIj0rfkli3EkHI5XqrS2XODs *)
