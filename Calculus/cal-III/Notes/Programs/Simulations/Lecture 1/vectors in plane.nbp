(* Content-type: application/mathematica *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 7.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       145,          7]
NotebookDataLength[     22597,        469]
NotebookOptionsPosition[      9892,        200]
NotebookOutlinePosition[     22684,        471]
CellTagsIndexPosition[     22641,        468]
WindowTitle->Section 12.1, Figure 12.06
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`showVectors$$ = {}, $CellContext`uVector$$ \
= {1, 0}, $CellContext`vVector$$ = {1, 1}, Typeset`show$$ = True, 
    Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`uVector$$], {1, 0}}, {-2, -2}, {2, 2}, {0.2, 0.2}}, {{
       Hold[$CellContext`vVector$$], {1, 1}}, {-2, -2}, {2, 2}, {0.2, 0.2}}, {{
       Hold[$CellContext`showVectors$$], {}, ""}, {
      1 -> "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"P\", \"\[VeryThinSpace]\
\", \"Q\"}], \"\[RightVector]\"],\n TraditionalForm]\)", 2 -> 
       "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"Q\", \"\[VeryThinSpace]\", \
\"P\"}], \"\[RightVector]\"],\n TraditionalForm]\)", 3 -> 
       "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"Q\", \"\[VeryThinSpace]\", \
\"R\"}], \"\[RightVector]\"],\n TraditionalForm]\)", 4 -> 
       "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"R\", \"\[VeryThinSpace]\", \
\"S\"}], \"\[RightVector]\"],\n TraditionalForm]\)"}}, {
      Hold[
       Pane[
       "Drag the symbols \[CirclePlus] to change \!\(\*\nStyleBox[\"u\",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)and\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\"]\)\!\(\*\
\nStyleBox[\".\",\nFontWeight->\"Bold\"]\)", {110, Automatic}, 
        BaseStyle -> {
         LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
          LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
          Left, FontSize -> 11}]], Manipulate`Dump`ThisIsNotAControl}}, 
    Typeset`size$$ = {432., {106., 111.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`uVector$267612$$ = {0, 
    0}, $CellContext`vVector$267613$$ = {0, 
    0}, $CellContext`showVectors$267614$$ = False}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`showVectors$$ = {}, $CellContext`uVector$$ = \
{1, 0}, $CellContext`vVector$$ = {1, 1}}, "ControllerVariables" :> {
        Hold[$CellContext`uVector$$, $CellContext`uVector$267612$$, {0, 0}], 
        Hold[$CellContext`vVector$$, $CellContext`vVector$267613$$, {0, 0}], 
        Hold[$CellContext`showVectors$$, $CellContext`showVectors$267614$$, 
         False]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Graphics[{Gray, 
         AbsoluteThickness[0.5], 
         Table[
          
          Line[{(-20) $CellContext`uVector$$ + $CellContext`i \
$CellContext`vVector$$, 
            20 $CellContext`uVector$$ + $CellContext`i \
$CellContext`vVector$$}], {$CellContext`i, -20, 20}], 
         Table[
          
          Line[{(-20) $CellContext`vVector$$ + $CellContext`i \
$CellContext`uVector$$, 
            20 $CellContext`vVector$$ + $CellContext`i \
$CellContext`uVector$$}], {$CellContext`i, -20, 20}], Black, 
         Arrowheads[{0, 0.03}], 
         AbsoluteThickness[2], 
         Arrow[{{0, 0}, $CellContext`uVector$$}], 
         Arrow[{{0, 0}, $CellContext`vVector$$}], 
         AbsolutePointSize[6], Black, 
         Point[{0, 0}], $CellContext`bcR, 
         Point[2 $CellContext`uVector$$], 
         Point[3 $CellContext`uVector$$], 
         Point[3 $CellContext`uVector$$ + 2 $CellContext`vVector$$], 
         Point[2 $CellContext`vVector$$], Black, 
         Text[
         "\!\(\*\nStyleBox[\"O\",\nFontSlant->\"Italic\"]\)", {0, 0}, {1.5, 
          1.5}], 
         Text[
         "\!\(\*\nStyleBox[\"P\",\nFontSlant->\"Italic\"]\)", 
          2 $CellContext`uVector$$, {1.5, 1.5}], 
         Text[
         "\!\(\*\nStyleBox[\"Q\",\nFontSlant->\"Italic\"]\)", 
          3 $CellContext`uVector$$, {-1.5, 1.5}], 
         Text[
         "\!\(\*\nStyleBox[\"R\",\nFontSlant->\"Italic\"]\)", 
          3 $CellContext`uVector$$ + 2 $CellContext`vVector$$, {-1.5, -1.5}], 
         
         Text[
         "\!\(\*\nStyleBox[\"S\",\nFontSlant->\"Italic\"]\)", 
          2 $CellContext`vVector$$, {1.5, -1.5}], 
         Text[
         "\!\(\*FormBox[\n StyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"],\n TraditionalForm]\)", (1/2) $CellContext`uVector$$, {
          0, 2}], 
         Text[
         "\!\(\*FormBox[\n StyleBox[\"v\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"],\n TraditionalForm]\)", (1/2) $CellContext`vVector$$, {
          2, 0}], 
         If[
          MemberQ[$CellContext`showVectors$$, 1], {$CellContext`bcB, 
           Arrow[{2 $CellContext`uVector$$, 3 $CellContext`uVector$$}], Black, 
           Text[
           "\!\(\*FormBox[\n RowBox[{OverscriptBox[\n   RowBox[{\"P\", \"\
\[VeryThinSpace]\", \"Q\"}], \"\[RightVector]\"], \"=\", \n  StyleBox[\"u\",\n\
FontWeight->\"Bold\",\nFontSlant->\"Plain\"]}],\n TraditionalForm]\)", (
             2 $CellContext`uVector$$ + 3 $CellContext`uVector$$)/2, {0, 2}]},
           Black], 
         If[
          MemberQ[$CellContext`showVectors$$, 2], {$CellContext`bcR, 
           Arrow[{3 $CellContext`uVector$$, 2 $CellContext`uVector$$}], Black, 
           Text[
           "\!\(\*FormBox[\n RowBox[{OverscriptBox[\n   RowBox[{\"Q\", \"\
\[VeryThinSpace]\", \"P\"}], \"\[RightVector]\"], \"=\", \n  RowBox[{\"-\", \n\
   StyleBox[\"u\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}]}],\n \
TraditionalForm]\)", (2 $CellContext`uVector$$ + 3 $CellContext`uVector$$)/
            2, {0, -2}]}, Black], 
         If[
          MemberQ[$CellContext`showVectors$$, 3], {$CellContext`bcB, 
           
           Arrow[{3 $CellContext`uVector$$, 3 $CellContext`uVector$$ + 
             2 $CellContext`vVector$$}], Black, 
           Text[
           "\!\(\*FormBox[\n RowBox[{OverscriptBox[\n   RowBox[{\"Q\", \"\
\[VeryThinSpace]\", \"R\"}], \"\[RightVector]\"], \"=\", \n  RowBox[{\"2\", \n\
   StyleBox[\"v\",\nFontWeight->\"Bold\",\nFontSlant->\"Plain\"]}]}],\n \
TraditionalForm]\)", 3 $CellContext`uVector$$ + $CellContext`vVector$$, {-2, 
            0}]}, Black], 
         If[
          MemberQ[$CellContext`showVectors$$, 4], {$CellContext`bcR, 
           
           Arrow[{3 $CellContext`uVector$$ + 2 $CellContext`vVector$$, 
             2 $CellContext`vVector$$}], Black, 
           Text[
           "\!\(\*FormBox[\n RowBox[{OverscriptBox[\n   RowBox[{\"R\", \"\
\[VeryThinSpace]\", \"S\"}], \"\[RightVector]\"], \"=\", \n  RowBox[{\n   \
RowBox[{\"-\", \"3\"}], \n   StyleBox[\"u\",\nFontWeight->\"Bold\",\n\
FontSlant->\"Plain\"]}]}],\n TraditionalForm]\)", 1.5 $CellContext`uVector$$ + 
            2 $CellContext`vVector$$, {0, -2}]}, Black]}, 
        PlotRange -> {{-2, 10}, {-2, 4}}, ImageSize -> 6 72, 
        BaseStyle -> {"Text", 13}, Axes -> True, Ticks -> None, AxesStyle -> 
        Arrowheads[{0, 0.03}]], 
      "Specifications" :> {{{$CellContext`uVector$$, {1, 0}}, {-2, -2}, {2, 
         2}, {0.2, 0.2}, ControlType -> 
         Locator}, {{$CellContext`vVector$$, {1, 1}}, {-2, -2}, {2, 2}, {0.2, 
         0.2}, ControlType -> 
         Locator}, {{$CellContext`showVectors$$, {}, ""}, {
         1 -> "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"P\", \"\
\[VeryThinSpace]\", \"Q\"}], \"\[RightVector]\"],\n TraditionalForm]\)", 2 -> 
          "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"Q\", \
\"\[VeryThinSpace]\", \"P\"}], \"\[RightVector]\"],\n TraditionalForm]\)", 3 -> 
          "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"Q\", \
\"\[VeryThinSpace]\", \"R\"}], \"\[RightVector]\"],\n TraditionalForm]\)", 4 -> 
          "\!\(\*FormBox[OverscriptBox[\n  RowBox[{\"R\", \
\"\[VeryThinSpace]\", \"S\"}], \"\[RightVector]\"],\n TraditionalForm]\)"}, 
         ControlType -> CheckboxBar, Appearance -> "Vertical"}, Delimiter, 
        Pane[
        "Drag the symbols \[CirclePlus] to change \!\(\*\nStyleBox[\"u\",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)and\!\(\*\nStyleBox[\" \",\n\
FontWeight->\"Bold\"]\)\!\(\*\nStyleBox[\"v\",\nFontWeight->\"Bold\"]\)\!\(\*\
\nStyleBox[\".\",\nFontWeight->\"Bold\"]\)", {110, Automatic}, 
         BaseStyle -> {
          LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
           LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
           Left, FontSize -> 11}]}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{594., {137.84375, 143.15625}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({$CellContext`i = 1, $CellContext`bcR = 
       RGBColor[0.8, 0, 0.4], $CellContext`bcB = RGBColor[0, 0.4, 0.8], 
       Attributes[PlotRange] = {ReadProtected}}; Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{800, 600},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 12.1, Figure 12.06",
Visible->True,
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"12.1 Vectors in the Plane\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.1 Vectors in the Plane\n", "SR", FontSize -> 11]}],
     CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.1 Vectors in the Plane\n", "SR", FontSize -> 11]}],
     CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"7.0 for Mac OS X x86 (32-bit) (February 18, 2009)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Text"], FontFamily -> "Times"]}, Visible -> False, 
  FrontEndVersion -> "7.0 for Mac OS X x86 (32-bit) (February 18, 2009)", 
  StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[585, 21, 9303, 177, 299, "Output"]
}
]
*)

(* End of internal cache information *)
(* NotebookSignature wJNz1W@3156MXK#BqCVQbtvc *)
