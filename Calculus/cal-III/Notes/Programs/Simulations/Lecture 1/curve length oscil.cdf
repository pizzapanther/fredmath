(* Content-type: application/vnd.wolfram.player *)

(*** Wolfram Notebook File ***)
(* http://www.wolfram.com/nb *)

(* CreatedBy='Mathematica 9.0' *)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       152,          7]
NotebookDataLength[     21498,        462]
NotebookOptionsPosition[      9085,        200]
NotebookOutlinePosition[     21592,        464]
CellTagsIndexPosition[     21549,        461]
WindowTitle->Section 12.8, Figure 12.p46
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`rValue$$ = 0.885, $CellContext`tValue$$ = 
    12.26, Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`tValue$$], 0, 
       "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)"}, 0, 
      Dynamic[4 (Pi/Dynamic[$CellContext`rValue$$])], 0.01}, {{
       Hold[$CellContext`tValue$$], 0, ""}, 0, 
      Dynamic[4 (Pi/Dynamic[$CellContext`rValue$$])], 0.01}, {
      Hold[
       Grid[{{
         "\!\(\*FormBox[\"t\",\n TraditionalForm]\)", 
          "\!\(\*FormBox[\"=\",\n TraditionalForm]\)", 
          Dynamic[
           Round[$CellContext`tValue$$, 0.01]]}, {
         "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"x\", \",\", \"y\"}], \
\")\"}],\n TraditionalForm]\)", "\!\(\*FormBox[\"=\",\n TraditionalForm]\)", 
          Dynamic[
           Row[{"(", 
             Round[
              $CellContext`funcLTC12FP46x[$CellContext`tValue$$, \
$CellContext`rValue$$], 0.01], ", ", 
             Round[
              $CellContext`funcLTC12FP46y[$CellContext`tValue$$, \
$CellContext`rValue$$], 0.01], ")"}]]}}, 
        Alignment -> {{Right, Center, Left}, Automatic}]], 
      Manipulate`Dump`ThisIsNotAControl}, {{
       Hold[$CellContext`rValue$$], 1, 
       "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, 0.5, 2}, {{
       Hold[$CellContext`rValue$$], 1, ""}, {
      1 -> "\!\(\*FormBox[\n RowBox[{\"a\", \"=\", \"1\"}],\n \
TraditionalForm]\)"}}}, Typeset`size$$ = {360., {109., 115.}}, 
    Typeset`update$$ = 0, Typeset`initDone$$, Typeset`skipInitDone$$ = 
    False, $CellContext`tValue$191966$$ = 0, $CellContext`rValue$191967$$ = 
    0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`rValue$$ = 1, $CellContext`tValue$$ = 0}, 
      "ControllerVariables" :> {
        Hold[$CellContext`tValue$$, $CellContext`tValue$191966$$, 0], 
        Hold[$CellContext`rValue$$, $CellContext`rValue$191967$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Show[{
         ParametricPlot[{
           $CellContext`funcLTC12FP46x[$CellContext`t, $CellContext`rValue$$], 
           $CellContext`funcLTC12FP46y[$CellContext`t, \
$CellContext`rValue$$]}, {$CellContext`t, -0.0001, 10 Pi}, 
          PlotStyle -> {{Gray}}], 
         ParametricPlot[{
           $CellContext`funcLTC12FP46x[$CellContext`t, $CellContext`rValue$$], 
           $CellContext`funcLTC12FP46y[$CellContext`t, \
$CellContext`rValue$$]}, {$CellContext`t, -0.0001, $CellContext`tValue$$}, 
          PlotStyle -> {{Thick, Black}}]}, 
        PlotRange -> {{0, 3.5 Pi}, {-0.5, 6}}, Ticks -> {None, 
          Range[0, 4]}, AxesOrigin -> {0, 0}, AxesStyle -> 
        Arrowheads[{0, 0.03}], 
        AxesLabel -> {
         "\!\(\*\nStyleBox[\"x\",\nFontSlant->\"Italic\"]\)", 
          "\!\(\*\nStyleBox[\"y\",\nFontSlant->\"Italic\"]\)"}, 
        BaseStyle -> {"Text", 13}, Epilog -> {
          Table[
           Text[
            
            Row[{$CellContext`i, 
              "\!\(\*FormBox[\n RowBox[{\"\[Pi]\", \" \", \"a\"}],\n \
TraditionalForm]\)"}], {(Pi $CellContext`i) $CellContext`rValue$$, 0}, {
            0, 1.5}], {$CellContext`i, 2, 6, 2}], 
          Text[
           Framed[
            Pane[
            " \!\(\*FormBox[\n RowBox[{\"x\", \"=\", \n  RowBox[{\"a\", \
\"(\", \n   RowBox[{\"t\", \"-\", \n    RowBox[{\"sin\", \" \", \"t\"}]}], \
\")\"}]}],\n TraditionalForm]\), \!\(\*FormBox[\n RowBox[{\"y\", \"=\", \n  \
RowBox[{\"a\", \"(\", \n   RowBox[{\"1\", \"-\", \n    RowBox[{\"cos\", \" \
\", \"t\"}]}], \")\"}]}],\n TraditionalForm]\),\nfor \!\(\*FormBox[\n \
RowBox[{\"t\", \"\[GreaterEqual]\", \"0\"}],\n TraditionalForm]\)", {
             Automatic, Automatic}, $CellContext`bcPBS], $CellContext`bcFO], {
           0, 6}, {-1.5, 1}], 
          AbsolutePointSize[4], $CellContext`bcG, 
          AbsoluteThickness[0.5], Dashed, 
          Line[{{0, $CellContext`rValue$$}, {3.5 Pi, $CellContext`rValue$$}}], 
          Dashing[{}], 
          AbsoluteThickness[1.5], 
          
          Circle[{$CellContext`rValue$$ $CellContext`tValue$$, \
$CellContext`rValue$$}, $CellContext`rValue$$], 
          
          Point[{$CellContext`rValue$$ $CellContext`tValue$$, \
$CellContext`rValue$$}], 
          
          Line[{{$CellContext`rValue$$ $CellContext`tValue$$, \
$CellContext`rValue$$}, {
             $CellContext`funcLTC12FP46x[$CellContext`tValue$$, \
$CellContext`rValue$$], 
             $CellContext`funcLTC12FP46y[$CellContext`tValue$$, \
$CellContext`rValue$$]}}], 
          AbsolutePointSize[6], $CellContext`bcR, 
          Point[{
            $CellContext`funcLTC12FP46x[$CellContext`tValue$$, \
$CellContext`rValue$$], 
            $CellContext`funcLTC12FP46y[$CellContext`tValue$$, \
$CellContext`rValue$$]}]}], 
      "Specifications" :> {{{$CellContext`tValue$$, 0, 
          "\!\(\*\nStyleBox[\"t\",\nFontSlant->\"Italic\"]\)"}, 0, 
         Dynamic[4 (Pi/Dynamic[$CellContext`rValue$$])], 0.01, ControlType -> 
         Slider, ImageSize -> {108, Automatic}}, {{$CellContext`tValue$$, 0, 
          ""}, 0, 
         Dynamic[4 (Pi/Dynamic[$CellContext`rValue$$])], 0.01, ControlType -> 
         Trigger, DefaultDuration -> 10, 
         AppearanceElements -> {
          "PlayPauseButton", "ResetButton", "StepLeftButton", 
           "StepRightButton"}}, 
        Grid[{{
          "\!\(\*FormBox[\"t\",\n TraditionalForm]\)", 
           "\!\(\*FormBox[\"=\",\n TraditionalForm]\)", 
           Dynamic[
            Round[$CellContext`tValue$$, 0.01]]}, {
          "\!\(\*FormBox[\n RowBox[{\"(\", \n  RowBox[{\"x\", \",\", \"y\"}], \
\")\"}],\n TraditionalForm]\)", "\!\(\*FormBox[\"=\",\n TraditionalForm]\)", 
           Dynamic[
            Row[{"(", 
              Round[
               $CellContext`funcLTC12FP46x[$CellContext`tValue$$, \
$CellContext`rValue$$], 0.01], ", ", 
              Round[
               $CellContext`funcLTC12FP46y[$CellContext`tValue$$, \
$CellContext`rValue$$], 0.01], ")"}]]}}, 
         Alignment -> {{Right, Center, Left}, 
           Automatic}], {{$CellContext`rValue$$, 1, 
          "\!\(\*\nStyleBox[\"a\",\nFontSlant->\"Italic\"]\)"}, 0.5, 2, 
         ControlType -> Slider, 
         ImageSize -> {108, Automatic}}, {{$CellContext`rValue$$, 1, ""}, {
         1 -> "\!\(\*FormBox[\n RowBox[{\"a\", \"=\", \"1\"}],\n \
TraditionalForm]\)"}, ControlType -> RadioButtonBar}}, 
      "Options" :> {
       ControlPlacement -> Left, LabelStyle -> 11, Deployed -> True, 
        AppearanceElements -> "ResetButton"}, "DefaultOptions" :> {}],
     ImageSizeCache->{535., {141., 146.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({{$CellContext`funcLTC12FP46x[
           Pattern[$CellContext`t, 
            Blank[]], 
           Pattern[$CellContext`r, 
            Blank[]]] := $CellContext`r ($CellContext`t - 
           Sin[$CellContext`t]), $CellContext`t[
           Pattern[$CellContext`df, 
            Blank[]]] := PDF[
           
           StudentTDistribution[$CellContext`df], $CellContext`x], \
$CellContext`funcLTC12FP46y[
           Pattern[$CellContext`t, 
            Blank[]], 
           Pattern[$CellContext`r, 
            Blank[]]] := $CellContext`r (1 - Cos[$CellContext`t]), 
         Attributes[PlotRange] = {ReadProtected}, $CellContext`i = 
         1, $CellContext`bcPBS = 
         BaseStyle -> {
           LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}, 
            LineBreakWithin -> Automatic, Hyphenation -> True, TextAlignment -> 
            Left, FontSize -> 13}, $CellContext`bcFO = {
          RoundingRadius -> 5, FrameStyle -> 
           GrayLevel[0.85]}, $CellContext`bcG = 
         RGBColor[0, 0.6, 0.4], $CellContext`bcR = RGBColor[0.8, 0, 0.4]}; {
        Null, Null}}; Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output"]
},
Editable->False,
Saveable->False,
Selectable->True,
WindowSize->{1309, 895},
WindowMargins->{{5, Automatic}, {Automatic, 5}},
WindowTitle->"Section 12.8, Figure 12.p46",
DockedCells->Cell[
  RawBoxes[
   TagBox[
    GridBox[{{
       PaneBox[
        GraphicsBox[
         RasterBox[CompressedData["
1:

          "], {{0, 0}, {90, 35}}, {0, 255}, ColorFunction -> RGBColor], 
         ImageSize -> {90, 35}, PlotRange -> {{0, 90}, {0, 35}}], 
        ImageMargins -> 0, ImageSize -> {90, Automatic}], 
       TagBox[
        GridBox[{{
           PaneBox[
            TemplateBox[{
              PaneBox[
               TemplateBox[{
                 StyleBox["\"CALCULUS\"", "SR", 18, StripOnInput -> False]}, 
                "Row", DisplayFunction -> (#& ), 
                InterpretationFunction -> (RowBox[{"Row", "[", 
                   RowBox[{"{", #, "}"}], "]"}]& )], 
               Alignment -> {Left, Center}, ImageSize -> {364, 22}], 
              PaneBox[
               StyleBox[
               "\"Briggs, Cochran, Gillett, Schulz\"", "SR", 12, Bold, 
                StripOnInput -> False], Alignment -> {Right, Center}, 
               ImageSize -> {244, 22}], 
              PaneBox[
               GraphicsBox[
                TagBox[
                 RasterBox[CompressedData["
1:
                  "], {{0, 43}, {110, 0}}, {0, 255}, 
                  ColorFunction -> RGBColor], 
                 BoxForm`ImageTag[
                 "Byte", ColorSpace -> "RGB", Interleaving -> True], 
                 Selectable -> False], BaseStyle -> "ImageGraphics", 
                ImageSize -> Automatic, ImageSizeRaw -> {110, 43}, 
                PlotRange -> {{0, 110}, {0, 43}}], ImageMargins -> 0, 
               Alignment -> {Right, Center}, ImageSize -> {71, 22}]}, "Row", 
             DisplayFunction -> (
              RowBox[{#, "\[InvisibleSpace]", #2, 
                "\[InvisibleSpace]", #3}]& ), 
             InterpretationFunction -> (RowBox[{"Row", "[", 
                RowBox[{"{", 
                  RowBox[{#, ",", #2, ",", #3}], "}"}], "]"}]& )], 
            ImageSize -> {682, 22}]}, {
           PaneBox[
            StyleBox[
            "\"12.8 Length of Curves\"", "SR", FontSize -> 12, Bold, 
             StripOnInput -> False], ImageMargins -> 0, 
            BaseStyle -> {
             LineIndent -> 0, LinebreakAdjustments -> {1, 1, 0, 0, 0}}, 
            ImageSize -> {573, Automatic}]}}, 
         GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
         AutoDelete -> False, 
         GridBoxItemSize -> {
          "Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}}, 
         GridBoxSpacings -> {"Columns" -> {{0.05}}, "Rows" -> {{0.05}}}], 
        "Grid"]}}, 
     GridBoxAlignment -> {"Columns" -> {{Left}}, "Rows" -> {{Top}}}, 
     AutoDelete -> False, 
     GridBoxItemSize -> {"Columns" -> {{Automatic}}, "Rows" -> {{Automatic}}},
      GridBoxSpacings -> {"Columns" -> {{0.5}}, "Rows" -> {{0.5}}}], "Grid"]],
   "DockedCell", Background -> GrayLevel[0.95]],
PageHeaders->{{
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.8 Length of Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox[
      "Chapter  12\nVectors and Vector-Valued Functions", "SR", FontSize -> 
       11]}], CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Section 12.8 Length of Curves\n", "SR", FontSize -> 11]}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}], 
   Cell[
    TextData[{
      StyleBox["Page ", "TR", 11], 
      StyleBox[
       CounterBox["Page"], "TR", 11], "  \n"}], 
    CellMargins -> {{Inherited, 0}, {Inherited, Inherited}}]}},
PageFooters->{{
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}, {
   Cell[
    TextData[{
      StyleBox["CALCULUS", "SR", FontSize -> 12], "", "\n", 
      StyleBox["Briggs, Cochran, Gillett, Schulz", "SR", 11]}], 
    CellMargins -> {{0, Inherited}, {Inherited, Inherited}}], 
   Cell[
    TextData[{"Printed: ", 
      ValueBox["Date"], "\n"}]], 
   Cell[
    TextData[{
      StyleBox[
      "Copyright \[Copyright] 2011, Pearson Education, Inc.", "TR", FontSize -> 
       10], "\n"}], CellMargins -> {{0, Inherited}, {Inherited, Inherited}}]}},
PrintingOptions->{"FacingPages"->False,
"FirstPageHeader"->True,
"PaperOrientation"->"Landscape"},
ShowCellBracket->False,
Deployed->False,
Copyable->False,
ShowCellTags->False,
Magnification->1,
FrontEndVersion->"9.0 for Microsoft Windows (64-bit) (November 20, 2012)",
StyleDefinitions->"Default.nb"
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[593, 21, 8488, 177, 304, "Output"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature FLjpHAjyWfkhAK5nGftfYDGy *)
