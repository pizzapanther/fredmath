...
%      g(x) = y_{n} + h/24(9(2x-3t(i+1))+19f_{n}-5f_{n-1}+f_{n-2}) - x
...
%--------------------------------------------------------------
    ...
    for i=4:steps
        t(i+1)=t(i)+h;
        str=sprintf('%g +%g/24*(9*(2*x - 3*%g)+19*%g-5*%g+%g)-x',y(i),h,t(i+1),fval(i),fval(i-1),fval(i-2));
