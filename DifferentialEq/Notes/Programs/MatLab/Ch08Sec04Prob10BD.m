%function Ch08Sec04Prob10BD
%
% Implements the Backward Differentation method for y(0.5), y(1.0, y(1.5) and y(2.0) 
% for the IVP
%       y' = 2t + exp(-ty), y(0) = 1.
...
disp('| for T = 0.5, 1.0, 1.5, and 2.0.                                        |');
disp('*------------------------------------------------------------------------*');
disp(sprintf(' '));
...
%--------------------------------------------------
% Do Backward-Differentiation four times -- once for each T-value
%--------------------------------------------------
for j = 1:4 
    steps=T(j)/h;
...
%--------------------------------------------------------------
    ...
    disp(sprintf('    y(%2.1f) is approximately %.7g',T(j),y(i)));
end